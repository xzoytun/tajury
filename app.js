...
async function digiflazzRequest(endpoint, payload = {}) {
  try {
    ...
    const response = await axios.post(`${DIGIFLAZZ_BASE_URL}/${endpoint}`, data, {
      timeout: 15000
    });
...
/**
 * Mask a username for privacy: show first 2 chars, replace rest with 'x'.
 */
function maskUsername(uname) {
  if (!uname) return uname;
  if (uname.length <= 2) return uname;
  return uname.slice(0, 2) + 'x'.repeat(uname.length - 1);
}
...
app.post('/webhook/digiflazz', async (req, res) => {
  try {
    ...
    // Fungsi Sensor Nomor (Privasi Member)
    const maskTarget = (num) => {
      const s = String(num);
      if (s.length < 8) return s; 
      return s.slice(0, 4) + 'xxxxx' + s.slice(-3);
    };

    // 1. Cari data transaksi di DB
    const trx = await dbGetAsync('SELECT * FROM ppob_transactions WHERE ref_id = ?', [ref_id]);
    
    if (trx) {
      // Update DB (Simpan status Kapital)
      await dbRunAsync('UPDATE ppob_transactions SET status = ?, sn = ? WHERE ref_id = ?', [status.toUpperCase(), sn || '', ref_id]);

      let userInfo = `<code>${trx.user_id}</code>`;
      try {
        const chat = await bot.telegram.getChat(trx.user_id);
        userInfo = chat.username ? `@${chat.username}` : `<a href="tg://user?id=${trx.user_id}">${chat.first_name}</a>`;
      } catch (e) {}
      ...
      if (statusLower === 'sukses' || statusLower === 'success') {
        ...
        bot.telegram.sendMessage(trx.user_id, msgUser, { parse_mode: 'HTML' }).catch(()=>{});

        if (GROUP_ID) {
          const groupMsg = `
━━━━━━━━━━━━━━━━━━━━━
<b>✅ PEMBELIAN PPOB SUCCESS</b>
━━━━━━━━━━━━━━━━━━━━━
<blockquote>📦 <b>Produk :</b> ${trx.sku}
🎯 <b>Tujuan :</b> <code>${targetSensor}</code>
💰 <b>Harga  :</b> Rp ${trx.price.toLocaleString('id-ID')}
🆔 <b>TRX ID :</b> <code>${ref_id}</code>
🎟 <b>SN     :</b> <code>${sn}</code></blockquote>
━━━━━━━━━━━━━━━━━━━━━
👤 <b>User  :</b> ${userInfo}
🆔 <b>ID    :</b> <code>${trx.user_id}</code>
🕒 <b>Waktu :</b> <code>${timestamp} WIB</code>`.trim();
          
          bot.telegram.sendMessage(GROUP_ID, groupMsg, { parse_mode: 'HTML' }).catch(()=>{});
        }
      } 
      
      // -------------------------------------------------------
      // 🔴 LOGIKA JIKA GAGAL (REFUND)
      // -------------------------------------------------------
      else if (statusLower === 'gagal' || statusLower === 'failure') {
        // Cek status database biar nggak refund double
        if (trx.status !== 'GAGAL') { 
...
async function renderAdminPPOB(ctx, page) {
  ...
  try {
    ...
    // 2. Ambil Data per halaman (Limit & Offset)
    const rows = await dbAllAsync(
      `SELECT * FROM ppob_transactions ORDER BY id DESC LIMIT ? OFFSET ?`,
      [limit, offset]
    );
    ...
    rows.forEach((r, i) => {
      ...
      report += `<blockquote>`;
      report += `👤 <b>User  :</b> <code>${r.user_id}</code>\n`;
    ...
    // Jika dipanggil dari tombol (action), kita EDIT pesan. Jika dari command, kita KIRIM baru.
    if (ctx.updateType === 'callback_query') {
      await ctx.editMessageText(report, options).catch(() => {});
    } else {
      await ctx.reply(report, options);
    }
...
async function sendMainMenu(ctx) {
  try {
    ...
    // 1. Ambil Data secara Paralel (Hapus reseller_level)
    ...
    keyboard.push([{ text: '📱 Menu PPOB', callback_data: 'menu_ppob' }]);
    ...
    // 3. Susun Teks (Komisi dihapus, PPOB ditambahkan)
    const text = `
━━━━━━━━━━━━━━━━━━━━━━━━━
...
• <b>Menu PPOB</b> : Pulsa, Kuota, PLN & Game
• <b>Menu Reseller</b> : Kelola akun & statistik</blockquote>
⚠️ <b>Tips Top Up:</b>
...
📊 <b>Status:</b> ${status} (${s.total_create_akun || 0}/${s.batas_create_akun || 0})`.trim();
    ...
    else logger.error('❌ Error Trial Menu: ' + error.message);
...
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    // --- 2. LOGIKA DISKON FLAT 40% ---
    const role = (user?.role || 'user').toLowerCase();
    const isReseller = role === 'reseller' || role === 'admin';
    // Langsung set 0.4 (40%) tanpa cek level lagi
    const diskonRate = isReseller ? 0.4 : 0; 
    ...
    for (let i = 0; i < currentServers.length; i += 2) {
      ...
      // Fungsi getFlag diasumsikan sudah ada di app.js Anda
      const flag1 = typeof getFlag === 'function' ? getFlag(currentServers[i].lokasi) : '🌐';
      row.push({
        text: `${flag1} ${currentServers[i].nama_server}`,
      ...
      if (currentServers[i + 1]) {
        const flag2 = typeof getFlag === 'function' ? getFlag(currentServers[i + 1].lokasi) : '🌐';
        row.push({
          text: `${flag2} ${currentServers[i + 1].nama_server}`,
    ...
    const serverCards = currentServers.map(s => {
      ...
      const flag = typeof getFlag === 'function' ? getFlag(s.lokasi) : '🌐';
    ...
    if (isReseller) {
      headerText += `<i>⭐ Akun Reseller (Diskon 40%)</i>\n\n`;
    } else {
      headerText += `\n`;
    }
...
function hitungHargaJual(hargaAsli) {
  ...
  // Skema pasaran biar gak kemahalan tapi gak rugi tenaga
  if (hargaAsli < 10000) {
    untung = 800;           
...
bot.action('menu_ppob', async (ctx) => {
  ...
  try {
    // 1. Ambil saldo user terbaru untuk ditampilkan di menu
    ...
    const categories = [
      ...
      { name: 'PLN', emoji: '⚡' },
      { name: 'E-Money', emoji: '💳' },
      { name: 'Masa Aktif', emoji: '⏳' },
      { name: 'Voucher', emoji: '🎟️' }
    ];

    // 2. Susun tombol kategori jadi 2 kolom (Biar Rapi di HP)
    const buttons = [];
    for (let i = 0; i < categories.length; i += 2) {
      const row = [
        { text: `${categories[i].emoji} ${categories[i].name}`, callback_data: `ppob_cat:${categories[i].name}` }
      ];
      if (categories[i + 1]) {
        row.push({ text: `${categories[i + 1].emoji} ${categories[i + 1].name}`, callback_data: `ppob_cat:${categories[i + 1].name}` });
      }
      buttons.push(row);
    }

    // 3. Tambahkan tombol fungsional tambahan
...
━━━━━━━━━━━━━━━━━━━━━━
👤 ID Anda : <code>${userId}</code>
💰 Saldo   : <b>Rp ${saldoUser.toLocaleString('id-ID')}</b>
━━━━━━━━━━━━━━━━━━━━━━
<i>Silakan pilih kategori layanan di bawah ini:</i>
`.trim();
...
async function renderUserPPOB(ctx, page) {
  ...
  try {
    ...
    if (totalData.count === 0) {
      return ctx.editMessageText("<b>📭 RIWAYAT KOSONG</b>\nKamu belum memiliki transaksi PPOB.", {
        parse_mode: 'HTML',
...
// 2. Pilih Brand berdasarkan Kategori (HANYA SATU HANDLER)
bot.action(/^ppob_cat:(.+)$/, async (ctx) => {
  const category = ctx.match[1];
  await ctx.answerCbQuery(`Loading ${category}...`).catch(() => {});
  
  const result = await getDigiProducts(); // Memanggil fungsi cache
  ...
  await ctx.editMessageText(`<b>Provider ${category}</b>\nPilih provider/operator:`, {
    parse_mode: 'HTML',
    reply_markup: { inline_keyboard: buttons }
  }).catch(() => {});
});

// --- FIX: PPOB Brand Action with Safe Regex ---
bot.action(/^ppob_brd:([^:]+):([^:]+)(?::(\d+))?$/, async (ctx) => {
  const category = ctx.match[1];
  const brand = ctx.match[2];
  const page = parseInt(ctx.match[3] || 1);
  
  await ctx.answerCbQuery(`Memuat ${brand} Hal ${page}...`).catch(() => {});

  const result = await getDigiProducts();
  if (result.status === 'error') {
    return ctx.reply('❌ Gagal mengambil produk. Silakan coba lagi nanti.');
  }

  // Filter dengan pengecekan ketat (Trim agar tidak ada spasi siluman)
  const allFiltered = result.data
    .filter(item => 
  ...
  if (allFiltered.length === 0) {
    return ctx.reply(`❌ Produk ${brand} tidak tersedia.`);
  }
  ...
  const offset = (page - 1) * limit;
  const currentItems = allFiltered.slice(offset, offset + limit);

  let messageText = `<b>🎁 PILIH PRODUK ${brand.toUpperCase()}</b>\n`;
  messageText += `<i>Halaman ${page} / ${totalPages}</i>\n\n`;
  ...
  // --- TOMBOL NAVIGASI (FIXED CALLBACK) ---
  const navRow = [];
  if (page > 1) {
    navRow.push({ text: '⬅️ Prev', callback_data: `ppob_brd:${category}:${brand}:${page - 1}` });
  }
  if (page < totalPages) {
    navRow.push({ text: 'Next ➡️', callback_data: `ppob_brd:${category}:${brand}:${page + 1}` });
  }
  if (navRow.length > 0) buttons.push(navRow);
  ...
  try {
    ...
    if (!err.description.includes("message is not modified")) {
      logger.error("PPOB Pagination Error: " + err.message);
    }
  }
});
...
bot.action('confirm_upgrade_reseller', async (ctx) => {
  ...
  try {
    ...
    try {
      await dbRunAsync('UPDATE users SET role = ? WHERE user_id = ?', ['reseller', userId]);
    } catch (dbErr) {
    ...
    const keyboard = {
      inline_keyboard: [
        ...
        [
          ...
          { text: '💰 Saldo Digi',    callback_data: 'admin_cek_digi' }
        ],
        [
          { text: 'Kembali',          callback_data: 'menu_adminreseller' }
        ]
...
async function renderAdminTopup(ctx, page) {
  ...
  try {
    ...
    if (totalData.count === 0) {
      return ctx.editMessageText("<b>📭 LOG KOSONG</b>\nBelum ada riwayat topup masuk.", {
        parse_mode: 'HTML',
    ...
    logs.forEach((t, i) => {
      ...
      text += `${num}. ${icon} <b>${sumber.toUpperCase()}</b>\n`;
      text += `<blockquote>`;
      text += `👤 <b>User  :</b> ${t.username || 'No Name'}\n`;
      text += `🆔 <b>ID    :</b> <code>${t.user_id}</code>\n`;
      text += `💰 <b>Nom   :</b> <b>Rp ${t.amount.toLocaleString('id-ID')}</b>\n`;
      text += `🕒 <b>Waktu :</b> ${waktuWIB} WIB`;
      text += `</blockquote>\n`;
    });
    ...
    const buttons = [];
    const navRow = [];
    if (page > 1) navRow.push({ text: '⬅️ Prev', callback_data: `admtopup_page_${page - 1}` });
    if (page < totalPages) navRow.push({ text: 'Next ➡️', callback_data: `admtopup_page_${page + 1}` });
    ...
    ctx.reply("❌ Gagal mengambil data log.");
  }
}
...
bot.action('menu_reseller', async (ctx) => {
  ...
  try {
    ...
    const row = await dbGetAsync('SELECT role, username FROM users WHERE user_id = ?', [userId]);

    if (!row || row.role !== 'reseller') {
      return ctx.reply('❌ <b>Akses Terbatas:</b> Menu ini hanya dapat diakses oleh mitra Reseller Official.', { parse_mode: 'HTML' });
    }
    ...
    const content = `
━━━━━━━━━━━━━━━━━━━━━━━━━
<b>💼 DASHBOARD MITRA RESELLER</b>
━━━━━━━━━━━━━━━━━━━━━━━━━
<b>👤 Informasi Lisensi Anda</b>
<blockquote>• <b>Status Mitra</b> : <code>Reseller Official</code>
• <b>Hak Akses</b>   : <code>Full Premium Access</code>
• <b>Privilage</b>    : <code>Diskon Flat 40% All Service</code>
• <b>Update Data</b> : ${escapeHtml(currentDate)} | ${escapeHtml(currentTime)}</blockquote>
<b>📢 Kebijakan & Keuntungan Reseller</b>
<blockquote>1. Mendapatkan harga khusus potong saldo 40% secara otomatis saat pembuatan akun.
2. Akses fitur <b>Export Data</b> untuk mempermudah pembukuan penjualan Anda.
3. Berhak mengikuti <b>Event Mingguan</b> dengan total hadiah saldo bagi reseller teraktif.
4. Monitoring masa aktif pelanggan lebih mudah melalui menu <b>List & Expired</b>.</blockquote>
━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Gunakan tombol navigasi di bawah untuk mengelola bisnis Anda secara profesional.</i>
`.trim();
...
bot.action(/reseller_top_all(?::(\d+))?/, async (ctx) => {
  try {
    ...
    if (!rows || rows.length === 0) {
      return ctx.editMessageText('<b>📭 BELUM ADA DATA</b>', {
        parse_mode: 'HTML',
...
bot.action(/reseller_riwayat(?::(\d+))?/, async (ctx) => {
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      return ctx.editMessageText('<b>📭 BELUM ADA RIWAYAT PENJUALAN</b>\n\nAnda belum melakukan transaksi penjualan.', {
        parse_mode: 'HTML',
        reply_markup: {
          inline_keyboard: [[{ text: '⬅️ KEMBALI', callback_data: 'menu_reseller' }]]
        }
...
⏱️ <b>Durasi   :</b> 60 Menit</blockquote>
━━━━━━━━━━━━━━━━━━━━━
👤 <b>User  :</b> ${mention}
...
bot.on('text', async (ctx, next) => {
  ...
  try {
    ...
    if (typeof state.step === 'string' && state.step.startsWith('exp_')) {
      ...
      // Reseller dapat flat diskon 40%
      const diskon     = user.role === 'reseller' ? 0.4 : 0;
...
━━━━━━━━━━━━━━━━━━━━━
<blockquote>💵 <b>Nominal    :</b> Rp ${Number(amount).toLocaleString('id-ID')}
🏧 <b>Metode     :</b> ${escapeHtml(payload.payment_method || 'QRIS')}
💳 <b>Saldo Baru :</b> Rp ${Number(userAfterTopUp.saldo || 0).toLocaleString('id-ID')}
📋 <b>Order ID   :</b> <code>${escapeHtml(order_id)}</code>
👑 <b>Role       :</b> ${escapeHtml((userAfterTopUp.role || 'user').toString().toUpperCase())}</blockquote>
━━━━━━━━━━━━━━━━━━━━━
👤 <b>User  :</b> ${userMention}
...
async function handleEditField(ctx, userStateData, data, field, fieldName, query) {
  ...
  try {
    ...
    if (!desc.includes('message is not modified')) {
      logger.error('❌ Error editMessageText di handleEditField: ' + (err.message || err));
    }
  }
}
...
🏧 <b>Metode     :</b> Orkut (QRIS)
💳 <b>Saldo Baru :</b> Rp ${userRow.saldo.toLocaleString('id-ID')}
📋 <b>Referensi  :</b> <code>${refId}</code></blockquote>
━━━━━━━━━━━━━━━━━━━━━
👤 <b>User  :</b> ${usernameLog}
🆔 <b>ID    :</b> <code>${deposit.userId}</code>
🕒 <b>Waktu :</b> <code>${timestamp} WIB</code>`.trim();
