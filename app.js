...
async function sendMainMenu(ctx) {
  ...
  [   
    { text: '🔄 Perpanjang', callback_data: 'renew_menu_pilihan' }, 
...
• Limitasi pembuatan <b>1 Akun / 3 Hari</b> per user.
...
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    if (!servers || servers.length === 0) {
      return ctx.reply('<b>⚠️ SERVER TIDAK TERSEDIA</b>\n<blockquote>Mohon maaf, saat ini tidak ada server yang aktif dalam database kami.</blockquote>', { parse_mode: 'HTML' });
    }
    ...
    let diskonRate = 0;
    if (isReseller) {
      if (resellerLevel === 'platinum') diskonRate = 0.4; // 40%
      else if (resellerLevel === 'gold') diskonRate = 0.3; // 30%
      else diskonRate = 0.2; // Silver/Default 20%
    }
    ...
    // --- 5. FORMAT SERVER CARDS (HARGA HARIAN & BULANAN) ---
    const serverCards = currentServers.map(s => {
      ...
      let pricingInfo = '';
      if (isReseller) {
        pricingInfo = `• Jual: <code>Rp${basePriceDaily.toLocaleString('id-ID')}/hari</code> | <code>Rp${basePriceMonthly.toLocaleString('id-ID')}/bln</code>
• Beli: <b>Rp${modalPriceDaily.toLocaleString('id-ID')}/hari</b> | <b>Rp${modalPriceMonthly.toLocaleString('id-ID')}/bln</b>`;
      } else {
...
• Quota/Bulan: <code>${quotaMonth}GB</code>
• Status: <b>${status}</b> (${s.total_create_akun || 0}/${s.batas_create_akun || 0})`.trim();
    }).join('\n');

    let headerText = `━━━━━━━━━━━━━━━━━━━━━━━━━\n<b>📋 LIST HALAMAN SERVER (${currentPage + 1}/${totalPages})</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    if (isReseller) {
      headerText += `<i>⭐ Akun: Reseller ${resellerLevel.toUpperCase()} (${diskonRate * 100}% Diskon)</i>\n`;
    }

    const footerText = `\n\n<i>Silakan pilih salah satu server di atas untuk melanjutkan proses pendaftaran.</i>`;
...
bot.action(/^trial_server_zivpn_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    // 🔐 RULE BARU: user cuma boleh 1x trial sebelum topup
  ...
  return bot.telegram.sendMessage(
    ...
    if (!check.allowed) {
      ...
      return bot.telegram.sendMessage(
        ...
        `_Jatah akan reset otomatis 3 hari setelah klaim terakhir._`,
        { parse_mode: 'Markdown' }
      );
    }

    // 2️⃣ AMBIL DATA SERVER
    const serverRow = await dbGetAsync('SELECT nama_server, domain, auth FROM Server WHERE id = ?', [serverId]);
...
`.trim();
    ...
    // 7️⃣ NOTIF KE GRUP (DISESUAIKAN AGAR KONSISTEN)
    if (typeof GROUP_ID !== 'undefined' && GROUP_ID) {
      const timestamp = new Date().toLocaleString('id-ID', { 
        timeZone: 'Asia/Jakarta',
...
╭─〔 <b>🎁 TRIAL ZIVPN</b> 〕
│
├─ 👤 <b>User:</b> ${userMention} (<i>${labelUser}</i>)
