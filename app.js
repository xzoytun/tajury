...
bot.action('renew_select', async (ctx) => {
  ...
  try {
    if (ctx.updateType === 'callback_query') {
      await ctx.answerCbQuery('🔍 Mengecek akun aktif...').catch(() => {});
    }
    ...
    for (const res of checks) {
      ...
      if (!data || data.status !== 'success' || !Array.isArray(data.data)) continue;

      const exist = data.data.some(line => {
        const m = line.match(/User:\s*([^\s|]+)/i);
        return m && m[1].toLowerCase() === acc.akun.toLowerCase();
      });
    ...
    if (!buttons.length) {
      return safeEdit(ctx,
        '<b>⚠️ TIDAK ADA AKUN AKTIF</b>\nAkun tidak ditemukan di server online.',
        {
          parse_mode: 'HTML',
          reply_markup: { inline_keyboard: [[{ text: '🔙 KEMBALI', callback_data: 'send_main_menu' }]] }
        }
    ...
    logger.warn('renew_select error: ' + err.message);
  }
});
