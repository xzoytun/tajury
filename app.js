...
━━━━━━━━━━━━━━━━━━━━━━━━━━━`.trim();
  ...
  keyboard = [
    ...
    [{ text: '🚀 Vless', callback_data: `${action}_vless` }, { text: '🛡️ Trojan', callback_data: `${action}_trojan` }],
    [{ text: '💎 Zivpn', callback_data: `${action}_zivpn` }],
    ...commonKeys
  ];
...
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    const serverCards = currentServers.map(s => {
      ...
      let pricingInfo = '';
      if (isReseller) {
        pricingInfo = `• Jual/hr : <code>Rp${basePrice.toLocaleString('id-ID')}</code>\n• Beli/hr : <b>Rp${modalPrice.toLocaleString('id-ID')}</b>`;
      } else {
        pricingInfo = `• Harga/hr: <b>Rp${basePrice.toLocaleString('id-ID')}</b>`;
      }

      return `
<blockquote>🌏 SERVER: ${escapeHTML(s.nama_server)}</blockquote>
Lokasi: <i>${escapeHTML(s.lokasi || 'Global')}</i>
${pricingInfo}
• Limit IP: <code>${s.iplimit || 1}</code> | Quota: <code>${quotaMonth}GB</code>
