...
async function renderRenewPage(ctx, userId, page) {
  ...
  try {
    if (ctx.updateType === 'callback_query') {
      await ctx.answerCbQuery('🔍 Memvalidasi akun aktif...').catch(() => {});
    }

    // 1. Ambil SEMUA riwayat akun user dari DB
    ...
    // 2. CEK AKTIVITAS KE SERVER
    const activeAccounts = [];
    const checks = await Promise.allSettled(
      rawAccounts.map(acc => 
        axios.get(`http://${acc.domain}:5888/list`, {
    ...
    for (const res of checks) {
      if (res.status === 'fulfilled' && res.value.data?.status === 'success') {
        const { acc, data } = res.value;
        const target = acc.akun.toLowerCase().trim();
        
        const exist = data.data.some(line => {
    ...
    if (totalActive === 0) {
      return safeEdit(ctx, '<b>⚠️ TIDAK ADA AKUN AKTIF</b>\nAkun tidak terdeteksi aktif di server.', {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🔙 KEMBALI', callback_data: 'send_main_menu' }]] }
      });
    }
    ...
    const paginatedItems = activeAccounts.slice(startIdx, startIdx + itemsPerPage);

    // FIX DI SINI: Menghapus .toUpperCase() dan merapikan tampilan
    const buttons = paginatedItems.map(acc => ([{
      // Menggunakan acc.akun asli dari database tanpa merubah case (huruf besar/kecil)
      text: `👤 ${acc.akun} | ${acc.nama_server}`,
    ...
    const navButtons = [];
    if (currentPage > 1) {
      navButtons.push({ text: '◀️ Prev', callback_data: `ren_page_${currentPage - 1}` });
    }
    if (currentPage < totalPages) {
      navButtons.push({ text: 'Next ▶️', callback_data: `ren_page_${currentPage + 1}` });
    }
    ...
    await safeEdit(ctx, 
      `<b>🔄 RENEW AKUN (LIVE)</b>\n` +
      `Ditemukan <b>${totalActive}</b> akun aktif.\n\n` +
    ...
    ctx.reply('❌ Terjadi kesalahan saat memuat data.').catch(() => {});
  }
}
...
bot.action(/^res_ren:(.+):(.+):(.+)$/, async (ctx) => {
  ...
  try {
    ...
    if (!server) {
      return safeEdit(ctx, '❌ <b>Server tidak ditemukan</b>', {
        parse_mode: 'HTML',
...
bot.action(/^view_list:(.+)$/, async (ctx) => {
  ...
  try {
    ...
    // ===============================
    // 1️⃣ Ambil akun reseller dari DB
    // ===============================
    const myAccounts = await dbAllAsync(
      `SELECT akun, SUM(hari) AS total_hari, MIN(created_at) AS tgl_awal
       FROM invoice_log
    ...
    if (!myAccounts.length) {
      return ctx.editMessageText(
        `<b>?? MY MEMBER: ${type.toUpperCase()}</b>
━━━━━━━━━━━━━━━━━━━━━━
<i>Anda belum memiliki member di protokol ini.</i>`,
    ...
    // ===============================
    // 2️⃣ Map akun reseller
    // ===============================
    const accountMap = new Map();
    for (const row of myAccounts) {
      accountMap.set(row.akun.toLowerCase().trim(), row);
    }
    ...
    let output = `<b>📂 MY MEMBER: ${type.toUpperCase()}</b>
👤 <b>Reseller:</b> ${escapeHtml(ctx.from.first_name || '-')}
━━━━━━━━━━━━━━━━━━━━━━`;

    let totalFound = 0;

    // ===============================
    // 4️⃣ Fetch server PARALLEL
    // ===============================
    const results = await Promise.allSettled(
      servers.map(srv =>
        axios.get(`http://${srv.domain}:5888/list`, {
          ...
          timeout: 5000
        }).then(res => ({ srv, data: res.data }))
      )
    );

    for (const res of results) {
      if (res.status !== 'fulfilled') {
        output += `\n🖥️ <b>Server:</b> ❌ Offline`;
        continue;
      }

      const { srv, data } = res.value;
      output += `\n\n🖥️ <b>Server: ${escapeHtml(srv.nama_server)}</b>\n`;

      if (!data || data.status !== 'success' || !Array.isArray(data.data)) {
        output += '<i>(Data tidak tersedia)</i>';
        continue;
      }
      ...
      for (const line of data.data) {
        ...
        const username = matchUser[1].toLowerCase().trim();
        if (!accountMap.has(username)) continue;

        foundHere++;
        totalFound++;

        const info = accountMap.get(username);
        const start = new Date(info.tgl_awal);
        const exp = new Date(start);
        exp.setDate(exp.getDate() + Number(info.total_hari));

        const expFmt = exp.toLocaleDateString('id-ID', {
          day: '2-digit',
          month: 'short',
          year: 'numeric'
        });

        const quota = line.match(/Q:\s*([^\s|]+)/);
        const usage = line.match(/Use:\s*([^\s|]+)/);

        output +=
          `• <code>${username}</code>\n` +
          `  └ Exp: <code>${expFmt}</code>` +
          `${quota ? ` | Q: <code>${quota[1]}</code>` : ''}` +
          `${usage ? ` | Use: <code>${usage[1]}</code>` : ''}\n`;
      }

      if (!foundHere) output += '<i>(Tidak ada member aktif)</i>';
    }

    if (!totalFound) {
      output += `\n\n<i>⚠️ Akun ada di database, namun belum terdeteksi aktif di server.</i>`;
    }
    ...
    ctx.reply('❌ Gagal mengambil data member.').catch(() => {});
  }
});
    ...
    if (stateKms && adminIds.includes(userId)) {
      ...
      if (stateKms.step === 'WAIT_AMOUNT') {
        ...
        try {
          ...
          ctx.reply(`🚀 **SUKSES!**\nKomisi *Rp ${amount.toLocaleString()}* ditambahkan ke *${targetName}*.`, { parse_mode: 'Markdown' });
          bot.telegram.sendMessage(targetId, `🎁 **Bonus Komisi!**\nAdmin telah menambahkan komisi manual ke saldo Anda sebesar *Rp ${amount.toLocaleString()}*.`, { parse_mode: 'Markdown' }).catch(() => {});
        } catch (err) {
