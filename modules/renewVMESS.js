...
async function renewvmess(username, exp, quota, limitip, serverId) {
  console.log(`⚙️ Renewing VMESS for ${username} | Exp: ${exp} days | Quota: ${quota} | IP Limit: ${limitip}`);
...
`.trim();
        ...
        .catch((e) => {
          console.error('❌ Vmess Renew Error:', e.message);
          resolve({ status: 'error', message: '❌ Gagal menghubungi server Vmess.' });
        });
    });
  });
}
