...
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    // --- 5. FORMAT SERVER CARDS (RAPI & BLOCKQUOTE) ---
    const serverCards = currentServers.map(s => {
      const basePrice = s.harga || 0;
      const modalPrice = Math.floor(basePrice * (1 - diskonRate));
      const displayPrice = isReseller ? modalPrice : basePrice;
      ...
      let pricingInfo = '';
      if (isReseller) {
        pricingInfo = `• Jual/hari : <code>Rp${basePrice.toLocaleString('id-ID')}</code>\n• Beli/hr : <b>Rp${modalPrice.toLocaleString('id-ID')}</b>`;
      } else {
        pricingInfo = `• Harga/hari: <b>Rp${basePrice.toLocaleString('id-ID')}</b>`;
      }
...
bot.action('renew_select', async (ctx) => {
  const userId = ctx.from.id;
  try {
    await ctx.answerCbQuery('Checking accounts on server...');
    
    const rawAccounts = await dbAllAsync(
      `SELECT i.akun, i.layanan, i.protocol, s.id as server_id, s.domain, s.auth
       FROM invoice_log i
       JOIN Server s ON i.layanan = s.nama_server
       WHERE i.user_id = ? 
       GROUP BY i.akun`, [userId]
    );
    ...
    for (const acc of rawAccounts) {
      try {
        const response = await axios.get(`http://${acc.domain}:5888/list?type=${acc.protocol.toLowerCase()}&auth=${acc.auth}`, { timeout: 3000 });
        
        if (response.data.status === 'success') {
          // Filter data: cari baris yang mengandung "User: nama_akun |"
          const isExist = response.data.data.some(item => {
            // Kita ambil bagian setelah "User: " dan sebelum " |"
            const match = item.match(/User:\s*([^\s|]+)/);
            return match && match[1].toLowerCase() === acc.akun.toLowerCase();
          });

          if (isExist) {
            validButtons.push([{
              text: `👤 ${acc.akun} | ${acc.layanan}`,
              callback_data: `res_ren:${acc.protocol.toLowerCase()}:${acc.akun}:${acc.server_id}`
            }]);
          }
        }
      } catch (e) {
        console.error(`Server ${acc.layanan} timeout/offline.`);
      }
    }

    if (validButtons.length === 0) {
      return ctx.editMessageText('<b>⚠️ TIDAK ADA AKUN</b>\nAkun kamu tidak ditemukan di server aktif.', {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🔙 KEMBALI', callback_data: 'send_main_menu' }]] }
      });
    }

    validButtons.push([{ text: '🔙 KEMBALI', callback_data: 'send_main_menu' }]);

    await ctx.editMessageText('<b>🔄 RENEW (LIVE DATA)</b>\nPilih akun yang ingin diperpanjang:', {
      parse_mode: 'HTML',
      reply_markup: { inline_keyboard: validButtons }
    });
  } catch (err) {
    console.error('Error reseller_renew_select:', err);
  }
});

// --- STEP 2: HANDLER KLIK USERNAME ---
bot.action(/^res_ren:(.+):(.+):(.+)$/, async (ctx) => {
  const type = ctx.match[1];        // protocol (vmess/vless/ssh/dll)
  const username = ctx.match[2];    // username akun
  const srvId = ctx.match[3];       // server ID
  const chatId = ctx.chat.id;

  try {
    await ctx.answerCbQuery();

    // Ambil detail server lengkap
    ...
    if (!server) {
      return ctx.editMessageText('❌ *Server tidak ditemukan!*', {
        parse_mode: 'Markdown',
        reply_markup: {
          inline_keyboard: [[{ text: '🔙 KEMBALI', callback_data: 'renew_select' }]]
        }
      });
    }

    // ✅ SEMUA PROTOCOL langsung ke input durasi hari
    userState[chatId] = {
      action: 'renew',
      type: type,
      username: username,
      password: username, // 🔥 Untuk ZIVPN, username = password
      serverId: srvId,
...
<i>Contoh: 30</i>
    ...
    logger.error('Error res_ren callback:', err);
    await ctx.answerCbQuery('❌ Terjadi kesalahan');
    await ctx.editMessageText('❌ *Terjadi kesalahan sistem*', {
      parse_mode: 'Markdown',
      reply_markup: {
...
bot.action(/^view_list:(.+)$/, async (ctx) => {
  ...
  try {
    await ctx.answerCbQuery(`🔍 Memverifikasi akun ${type.toUpperCase()}...`);
    
    // 1. QUERY AKUMULASI (Identik dengan Dashboard Akurat)
    const myAccountsRaw = await dbAllAsync(
      `SELECT LOWER(akun) as akun, SUM(hari) as total_hari, MIN(created_at) as tgl_awal 
       FROM invoice_log 
       WHERE user_id = ? AND LOWER(protocol) = ? 
       GROUP BY LOWER(akun)`, 
      [userId, type]
    );

    if (!myAccountsRaw || myAccountsRaw.length === 0) {
      return ctx.editMessageText(`<b>📂 MY MEMBER: ${type.toUpperCase()}</b>\n━━━━━━━━━━━━━━━━━━━━━━\n<i>Anda belum memiliki member di protokol ini.</i>`, {
        parse_mode: 'HTML',
    ...
    myAccountsRaw.forEach(row => {
      accountMap.set(row.akun.trim(), {
    ...
    let message = `<b>📂 MY MEMBER: ${type.toUpperCase()}</b>\n`;
    message += `👤 <b>Reseller:</b> ${ctx.from.first_name}\n`;
    message += '━━━━━━━━━━━━━━━━━━━━━━\n';

    for (const srv of servers) {
      try {
        const response = await axios.get(`http://${srv.domain}:5888/list?type=${type}&auth=${srv.auth}`, { timeout: 5000 });
        message += `\n🖥️ <b>Server: ${srv.nama_server}</b>\n`;
        
        if (response.data.status === 'success' && Array.isArray(response.data.data)) {
          let foundCount = 0;
          
          response.data.data.forEach((item) => {
            // REGEX disesuaikan dengan output shell baru: "User: nama | Exp: tgl | Q: ... | Use: ..."
            const matchUser = item.match(/User:\s*([^\s|]+)/);
            const vpsUser = matchUser ? matchUser[1].toLowerCase().trim() : "";

            if (accountMap.has(vpsUser)) {
              foundCount++;
              const info = accountMap.get(vpsUser);
              
              // LOGIKA HITUNG TANGGAL (Database Bot)
              const startDate = new Date(info.tgl_awal);
              const expiredDate = new Date(startDate);
              expiredDate.setDate(expiredDate.getDate() + parseInt(info.total_hari));
              
              const tglFmt = expiredDate.toLocaleDateString('id-ID', { 
                day: '2-digit', month: 'short', year: 'numeric' 
              });

              // AMBIL INFO TAMBAHAN DARI SHELL (Quota & Usage jika ada)
              const matchQ = item.match(/Q:\s*([^\s|]+)/);
              const matchUse = item.match(/Use:\s*([^\s|]+)/);
              const quotaInfo = matchQ ? ` | Q: <code>${matchQ[1]}</code>` : "";
              const usageInfo = matchUse ? ` | Use: <code>${matchUse[1]}</code>` : "";

              message += `${foundCount}. <code>${vpsUser}</code>\n└ Exp: <code>${tglFmt}</code>${quotaInfo}${usageInfo}\n`;
            }
          });
          
          if (foundCount === 0) message += '<i>(Tidak ada member anda)</i>\n';
        } else {
          message += '<i>(Data tidak tersedia)</i>\n';
        }
      } catch (e) {
        message += `\n🖥️ <b>Server: ${srv.nama_server}</b> (❌ Offline)\n`;
      }
    }
    ...
    console.error('Error view_list:', err);
...
╭─〔 <b>🎁 TRIAL ZIVPN</b> 〕
│
├─ 👤 <b>User:</b> ${userMention} (<i>${check.role.toUpperCase()}</i>)
...
// Fungsi Masking Username - Hanya 1 huruf depan + 'x' sebanyak sisa hurufnya + 2 x tambahan
function maskUsername(name) {
  if (!name || typeof name !== 'string') return name;
  
  const n = name.length;

  // Jika cuma 1 huruf, biarkan saja
  if (n <= 1) return name;

  const firstChar = name[0];
  const stars = 'x'.repeat(n - 1 + 2); // Tambah 2 x di sini
        ...
        async (err, row) => {
          ...
          db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', 
            ...
            async function(err) {
              ...
              // --- AMBIL USERNAME DARI DATABASE (Lebih Aman & Cepat) ---
...
try {
  ...
  if (userRow) {
    usernameLog = userRow.username ? `@${userRow.username}` : (userRow.first_name || String(deposit.userId));
  }
} catch (e) {
  logger.error('Gagal ambil username dari DB di processMatchingPayment:', e);
}
