...
cron.schedule('0 1 1 1/2 *', async () => {
  try {
    ...
    logger.info('✅ Proses arsip & reset komisi (2 bulanan) selesai.');
  } catch (err) {
    logger.error('❌ Gagal job reset komisi 2 bulanan: ' + (err.message || err));
  }
});
...
// Script Tambal Kolom (Hapus setelah bot jalan normal)
dbRunAsync("ALTER TABLE invoice_log ADD COLUMN protocol TEXT;").then(() => {
    console.log("✅ Kolom 'protocol' berhasil ditambahkan!");
}).catch(err => {
    console.log("⚠️ Kolom 'protocol' mungkin sudah ada.");
});
...
db.run(`ALTER TABLE topup_log ADD COLUMN metode TEXT`, (err) => {
  // Jika error karena kolom sudah ada, biarkan saja
});
...
// Fungsi untuk memformat tanggal expired (Asumsi: created_at + masa aktif hari)
...
// ==========================================
// 🛠️ FUNGSI AUTO-FIX DATABASE (MIGRASI)
// ==========================================
async function fixDatabaseStructure() {
  try {
    // 1. Cek kolom apa saja yang ada di invoice_log
    const columns = await dbAllAsync("PRAGMA table_info(invoice_log)");
    const hasProtocol = columns.some(col => col.name === 'protocol');

    // 2. Kalau kolom 'protocol' belum ada, kita tambahkan
    if (!hasProtocol) {
      console.log("⚙️  Database: Kolom 'protocol' tidak ditemukan, sedang menambahkan...");
      await dbRunAsync("ALTER TABLE invoice_log ADD COLUMN protocol TEXT");
      console.log("✅ Database: Kolom 'protocol' berhasil ditambahkan!");
    } else {
      console.log("✅ Database: Struktur sudah sesuai (Kolom 'protocol' sudah ada).");
    }
  } catch (err) {
    console.error("❌ Database Error saat cek struktur:", err.message);
  }
}

// Jalankan fungsi fix di atas
...
async function canTakeTrial(userId) {
  // Ambil data user termasuk saldo dari database
  const user = await dbGetAsync(
    'SELECT role, saldo, last_trial_date, trial_count_today FROM users WHERE user_id = ?',
    [userId]
  );

  if (!user) return { allowed: false, reason: 'USER_NOT_FOUND' };

  // --- SYARAT SALDO MINIMAL 2000 ---
  if (user.saldo < 2000) {
    return { allowed: false, reason: 'SALDO_INSUFFICIENT' };
  }
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN IMMEDIATE', async (err) => {
        ...
        // Ambil data user terbaru di dalam transaksi
        db.get('SELECT saldo, role, last_trial_date, trial_count_today FROM users WHERE user_id = ?', [userId], async (err, user) => {
          if (err || !user) {
            db.run('ROLLBACK');
            return resolve({ ok: false, reason: 'USER_NOT_FOUND' });
          }

          // Validasi saldo minimal 2000
          if (user.saldo < 2000) {
            db.run('ROLLBACK');
            return resolve({ ok: false, reason: 'SALDO_INSUFFICIENT' });
          }
          ...
          let diffDays = Infinity;
          if (last) {
            diffDays = Math.floor((new Date() - new Date(`${last}T00:00:00Z`)) / (1000 * 60 * 60 * 24));
          }

          let trialCount = diffDays >= 2 ? 0 : user.trial_count_today || 0;
          if (trialCount >= maxTrial) {
            db.run('ROLLBACK');
            return resolve({ ok: false, reason: 'LIMIT_REACHED' });
          }
...
async function sendMainMenu(ctx) {
  try {
    ...
    // 1. Ambil Data secara Paralel (Biar Gak Lag)
    const [userData, totalAkun, totalUserBot] = await Promise.all([
      ...
      dbGetAsync('SELECT COUNT(*) AS total FROM users')
    ]);
    ...
    // 4. Susun Keyboard 2 Kolom (Optimasi Jempol)
    const keyboard = [];

    if (role === 'admin' || adminIds.includes(String(userId))) {
      keyboard.push([{ text: '⚙️ MENU SYSTEM ADMIN', callback_data: 'menu_adminreseller' }]);
    }
    if (role === 'reseller') {
      keyboard.push([{ text: '💼 MENU RESELLER', callback_data: 'menu_reseller' }]);
    }
    ...
    // 5. Format Tampilan dengan Blockquote (Super Rapi)
    const text = `
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
• Total User Bot : <code>${totalUser} Member</code></blockquote>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 <b>Menu Tersedia</b>
<blockquote>• <b>Buat Akun</b> : Buat akun VPN baru (SSH, VMess, VLess, Trojan, ZiVPN)
• <b>Trial Akun</b> : Coba gratis 1 jam
• <b>Perpanjang</b> : Extend masa aktif akun
• <b>Top up Saldo</b> : Isi saldo untuk transaksi
• <b>Menu Reseller</b> : Lihat komisi & statistik (Khusus Reseller)</blockquote>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
`.trim();
    ...
    // 6. Eksekusi Kirim (Anti-Error)
    const options = { parse_mode: 'HTML', reply_markup: { inline_keyboard: keyboard } };

    if (ctx.updateType === 'callback_query') {
      await ctx.answerCbQuery().catch(() => {});
      await ctx.editMessageText(text, options).catch((e) => {
        if (!e.description?.includes('message is not modified')) logger.error(e.message);
      });
    } else {
    ...
    if (ctx.updateType !== 'callback_query') {
      ctx.reply('❌ Gagal menampilkan menu, database sedang sibuk.');
    }
  }
}
// ==========================================
...
// ==========================================
// 📋 GENERATE SERVICE MENU (PREMIUM COPYWRITING)
// ==========================================
function generateServiceMenu(action) {
  let keyboard = [];
  let teks = '';

  const commonKeys = [[{ text: '🏠 KEMBALI KE DASHBOARD UTAMA', callback_data: 'send_main_menu' }]];
  
  const menuConfig = {
    create: {
      title: '🚀 PEMBUATAN AKUN VPN PREMIUM',
      benefit: '• <b>High Performance:</b> Server menggunakan NVMe SSD.\n• <b>Multi-Path:</b> Mendukung berbagai jaringan ISP.\n• <b>Instant Active:</b> Akun langsung aktif setelah diproses.',
      desc: 'Silakan tentukan protokol VPN yang ingin Anda gunakan. Setiap protokol memiliki keunggulan tersendiri dalam menembus sensor internet.'
    },
    renew: {
      title: '🔄 LAYANAN PERPANJANGAN AKUN',
      benefit: '• <b>No Reconfig:</b> Tidak perlu setting ulang aplikasi.\n• <b>Loyalty Bonus:</b> Perpanjang sebelum mati untuk kestabilan.\n• <b>Seamless:</b> Transisi masa aktif tanpa pemutusan koneksi.',
      desc: 'Masukkan atau pilih jenis protokol akun yang ingin Anda perpanjang masa aktifnya agar tetap dapat digunakan.'
    },
    trial: {
      title: '🎁 AKSES TRIAL GRATIS (BETA TEST)',
      benefit: '• <b>Full Speed:</b> Kecepatan tidak dibatasi (Uncapped).\n• <b>Testing Ground:</b> Cek stabilitas sebelum membeli premium.\n• <b>Global Server:</b> Akses ke server publik kualitas premium.',
      desc: 'Coba kualitas infrastruktur kami selama 60 menit tanpa biaya sepeserpun. Rasakan perbedaannya sekarang juga!'
    }
  };
...
<b>${cfg.title}</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<blockquote>💡 <b>Mengapa Memilih Kami?</b>
${cfg.benefit}</blockquote>

<b>📡 Konfigurasi Protokol:</b>
<i>${cfg.desc}</i>
━━━━━━━━━━━━━━━━━━━━━━━━━━━`.trim();

  keyboard = [
    [{ text: '🔐 SSH TUNNEL', callback_data: `${action}_ssh` }, { text: '⚡ VMESS V2RAY', callback_data: `${action}_vmess` }],
    [{ text: '🚀 VLESS XTLS', callback_data: `${action}_vless` }, { text: '🛡️ TROJAN GFW', callback_data: `${action}_trojan` }],
    [{ text: '💎 ZIVPN PREMIUM', callback_data: `${action}_zivpn` }],
    ...commonKeys
  ];
...
// ==========================================
// ⌛ TRIAL SERVER MENU (PREMIUM LOOK)
// ==========================================
async function showTrialServerMenu(ctx, jenis) {
  try {
    ...
    // Query ringan
    const servers = await dbAllAsync('SELECT id, nama_server, lokasi FROM Server ORDER BY id ASC');
    
    if (!servers || servers.length === 0) {
      return ctx.reply('<b>⚠️ SERVER TIDAK TERSEDIA</b>\n<blockquote>Mohon maaf, saat ini kapasitas server trial sedang penuh atau dalam pemeliharaan rutin.</blockquote>\nSilakan coba lagi dalam beberapa saat.', { parse_mode: 'HTML' });
    }

    const keyboard = [];
    for (let i = 0; i < servers.length; i += 2) {
      const row = [];
      row.push({ text: `${servers[i].nama_server}`, callback_data: `trial_server_${jenis}_${servers[i].id}` });
      if (servers[i + 1]) {
        row.push({ text: `${servers[i + 1].nama_server}`, callback_data: `trial_server_${jenis}_${servers[i + 1].id}` });
      }
      keyboard.push(row);
    }
    ...
    const pesan = `
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<b>⌛ PEMILIHAN SERVER TRIAL ${jenis.toUpperCase()}</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<blockquote>Kami menyediakan infrastruktur server berkualitas tinggi dengan latensi rendah (Low Ping) khusus untuk pengujian performa jaringan Anda.</blockquote>

<b>📝 Syarat & Ketentuan Trial:</b>
• Durasi aktif akun adalah <b>60 Menit</b>.
• Limitasi pembuatan <b>1 Akun / 2 Hari</b> per user.
• Dilarang menggunakan trial untuk aktivitas ilegal (DDOS/Hacking).

<b>🌐 Silakan pilih lokasi server terbaik:</b>`.trim();
    ...
    logger.error(`❌ Gagal trial menu: ${err.message}`);
    ctx.reply('❌ Terjadi gangguan komunikasi dengan database.').catch(() => {});
  }
}
...
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    logger.info(`Memulai proses ${action} untuk ${type} (halaman ${page + 1}) oleh user ${userId}`);

    // --- 1. AMBIL ROLE PENGGUNA DAN LEVEL RESELLER ---
    // Pastikan Anda memiliki kolom 'reseller_level' di tabel 'users'
    const user = await dbGetAsync('SELECT role, reseller_level FROM users WHERE user_id = ?', [userId]);
    const role = user ? user.role.toLowerCase() : 'user'; // Default ke 'user'
    const isReseller = role === 'reseller' || role === 'admin';

    // HITUNG TINGKAT DISKON BERDASARKAN KONSEP USER
    const resellerLevel = user.reseller_level ? user.reseller_level.toLowerCase() : '';
    
    const diskonRate = isReseller
        ? (resellerLevel === 'gold' ? 0.3 // 30%
          : resellerLevel === 'platinum' ? 0.4 // 40%
          : 0.2) // Default reseller (atau level lain) 20%
        : 0; // User biasa 0%


    // --- 2. AMBIL DATA SERVER ---
    // Query tidak perlu mengambil 'harga_reseller' lagi
    ...
    const navButtons = [];
    if (currentPage > 0) {
      navButtons.push({ text: '🔙 Back', callback_data: `Maps_${action}_${type}_${currentPage - 1}` });
    }
    ...
    // --- 3. FORMAT SERVER CARDS DENGAN LOGIKA DISKON ---
    const serverCards = currentServers.map(s => {
      // Harga dasar (user biasa / harga jual reseller)
      const basePricePerDay = s.harga || 0;
      
      // Harga beli reseller (modal)
      const discountedPricePerDay = Math.floor(basePricePerDay * (1 - diskonRate));
      
      // Tentukan harga per hari yang akan ditampilkan
      const pricePerDay = isReseller
        ? discountedPricePerDay // Reseller melihat harga modal
        : basePricePerDay;   // User melihat harga jual

      const harga30 = pricePerDay * 30;
      ...
      if (isReseller) {
        // Tampilan khusus untuk Reseller
        const userPricePerDay = basePricePerDay;
        const userHarga30 = userPricePerDay * 30;

        priceText = `• Harga Jual : <code>Rp${userPricePerDay.toLocaleString('id-ID')}</code>\n` +
                    `• Harga Beli : <code>Rp${pricePerDay.toLocaleString('id-ID')}</code>`;
        
        price30Text = `• Jual/30h   : <code>Rp${userHarga30.toLocaleString('id-ID')}</code>\n` +
                      `• Beli/30h   : <code>Rp${harga30.toLocaleString('id-ID')}</code>`;
      } else {
        // Tampilan untuk User biasa
        priceText = `• Harga/hari : <code>Rp${pricePerDay.toLocaleString('id-ID')}</code>`;
        price30Text = `• Harga/30h  : <code>Rp${harga30.toLocaleString('id-ID')}</code>`;
      }
      ...
      return (
        ...
        `${priceText}\n` + // Baris harga per hari yang sudah disesuaikan
        `• Kuota/hari : <code>${escapeHTML(String(s.quota || '-'))} GB</code>\n` +
        `${price30Text}\n` + // Baris harga per 30 hari yang sudah disesuaikan
    ...
    let text = `<b>📋 List Server (Hal ${currentPage + 1}/${totalPages})</b>\n\n`;
    
    // Tambahkan notifikasi role di header
    if (isReseller) {
        // Tampilkan level reseller dan diskon
        const levelText = resellerLevel ? `Level ${resellerLevel.toUpperCase()}` : 'Default';
        text += `<i>⭐ Anda adalah *Reseller ${levelText}* (${diskonRate * 100}% Diskon). Harga Beli ditampilkan di bawah.</i>\n\n`;
    }
...
bot.action('menu_adminreseller', async (ctx) => {
  try {
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: '🖥️ MENU SERVER', callback_data: 'admin_server_menu' }, { text: '⚙️ MENU SISTEM', callback_data: 'admin_system_menu' }],
...
bot.action('admin_server_menu', async (ctx) => {
  try {
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: '➕ TAMBAH', callback_data: 'addserver' }, { text: '🗑️ HAPUS', callback_data: 'deleteserver' }],
        [{ text: '💰 HARGA', callback_data: 'editserver_harga' }, { text: '📝 NAMA', callback_data: 'nama_server_edit' }],
        [{ text: '🌐 DOMAIN', callback_data: 'editserver_domain' }, { text: '🔑 AUTH', callback_data: 'editserver_auth' }],
        [{ text: '📊 QUOTA', callback_data: 'editserver_quota' }, { text: '🔢 LIMIT IP', callback_data: 'editserver_limit_ip' }],
        [{ text: '💳 +SALDO', callback_data: 'addsaldo_user' }, { text: '💸 -SALDO', callback_data: 'reducesaldo_user' }],
        [{ text: '📋 DETAIL', callback_data: 'detailserver' }, { text: '⚠️ BATAS CRE', callback_data: 'editserver_batas_create_akun' }],
        [{ text: '📈 TOT CREATE', callback_data: 'editserver_total_create_akun' }, { text: '📑 LIST', callback_data: 'listserver' }],
        [{ text: '🔄 RESET DB', callback_data: 'resetdb' }, { text: '⬅️ KEMBALI', callback_data: 'menu_adminreseller' }]
      ]
    };
...
bot.action('admin_system_menu', async (ctx) => {
  try {
    ...
    const keyboard = {
      inline_keyboard: [
        [{ text: '📊 STATS', callback_data: 'admin_stats' }, { text: '👥 USERS', callback_data: 'admin_listuser' }],
        [{ text: '📢 BROADCAST', callback_data: 'admin_broadcast' }, { text: '💾 BACKUP', callback_data: 'admin_backup_db' }],
        [{ text: '♻️ RESTORE', callback_data: 'admin_restore2_db' }, { text: '📦 ALL BACKUP', callback_data: 'admin_restore_all' }],
        [{ text: '⬆️ UP RESELLER', callback_data: 'admin_promote_reseller' }, { text: '⬇️ DOWN', callback_data: 'admin_downgrade_reseller' }],
        [{ text: '🔧 LEVEL', callback_data: 'admin_ubah_level' }, { text: '💼 LIST RES', callback_data: 'admin_listreseller' }],
        [{ text: '🔄 RESET COM', callback_data: 'admin_resetkomisi' }, { text: '🎁 RESET TRIAL', callback_data: 'admin_reset_trial' }],
        [{ text: '📝 LOG TOPUP', callback_data: 'admin_view_topup' }],
...
bot.action('menu_reseller', async (ctx) => {
  ...
  inline_keyboard: [
    [
      { text: '📊 Statistik', callback_data: 'reseller_riwayat' },
    ...
    [
      ...
      { text: '🔄 Perpanjang Akun', callback_data: 'reseller_renew_select' } // Tambahan ini
    ],
    [
...
<blockquote>• <b>Statistik</b>
  Lihat riwayat transaksi & performa
• <b>Cek Komisi</b>
  Total komisi yang telah diperoleh
• <b>Export Data</b>
  Download data komisi ke file
• <b>Ranking Reseller</b>
  Pantau posisi di leaderboard</blockquote>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// --- STEP 1: PILIH AKUN ---
bot.action('reseller_renew_select', async (ctx) => {
  const userId = ctx.from.id;
  try {
    await ctx.answerCbQuery();
    
    // Query akurat: Ambil data dari akun_aktif JOIN dengan Server untuk dapat Nama Server
    const accounts = await dbAllAsync(
      `SELECT a.username, a.jenis, s.nama_server, s.id as server_id
       FROM akun_aktif a 
       JOIN invoice_log i ON a.username = i.akun 
       JOIN Server s ON i.layanan = s.nama_server
       WHERE i.user_id = ? 
       GROUP BY a.username 
    ...
    if (!accounts || accounts.length === 0) {
      return ctx.editMessageText('<b>⚠️ PERHATIAN!</b>\nKamu belum memiliki akun aktif untuk diperpanjang.', {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🔙 KEMBALI', callback_data: 'send_main_menu' }]] }
      });
    }

    const buttons = accounts.map(acc => ([{
      text: `👤 ${acc.username} | ${acc.nama_server}`,
      callback_data: `res_ren:${acc.jenis}:${acc.username}:${acc.server_id}`
    }]));

    buttons.push([{ text: '🔙 KEMBALI', callback_data: 'menu_reseller' }]);

    await ctx.editMessageText('<b>🔄 RENEW PANEL RESELLER</b>\n<i>Pilih akun yang ingin diperpanjang:</i>', {
      parse_mode: 'HTML',
      reply_markup: { inline_keyboard: buttons }
    });
  } catch (err) {
    logger.error('Error reseller_renew_select: ' + err.message);
  }
});
...
bot.action(/^res_ren:(.+):(.+):(.+)$/, async (ctx) => {
  ...
  try {
    ...
    const namaServer = server ? server.nama_server : 'Unknown Server';
...
🚀 <b>Server   :</b> <code>${namaServer}</code>
💰 <b>Saldo Mu :</b> <code>Rp ${(user?.saldo || 0).toLocaleString('id-ID')}</code>
━━━━━━━━━━━━━━━━━━━━━━
⏳ <b>Masukkan tambahan masa aktif (hari):</b>
<i>Contoh: Balas dengan angka <b>30</b></i>
    `.trim();
    ...
    await ctx.editMessageText(msg, {
      parse_mode: 'HTML',
      reply_markup: {
        inline_keyboard: [[{ text: '❌ BATALKAN', callback_data: 'reseller_renew_select' }]]
      }
    });
  } catch (err) {
    logger.error('Error res_ren_confirm: ' + err.message);
  }
});
...
bot.action('reseller_list_akun', async (ctx) => {
  ...
  try {
    ...
    // QUERY DIAKURKAN: Ambil kolom 'protocol' yang baru kamu tambahkan di INSERT
    const accounts = await dbAllAsync(
      `SELECT akun, layanan, protocol, SUM(hari) as total_hari, MIN(created_at) as tgl_awal 
       FROM invoice_log 
       WHERE user_id = ? 
       GROUP BY akun 
    ...
    if (!accounts || accounts.length === 0) {
      return ctx.editMessageText('<b>⚠️ PERHATIAN!</b>\nKamu belum memiliki riwayat akun.', {
        parse_mode: 'HTML',
        reply_markup: {
    ...
    const servers = await dbAllAsync('SELECT nama_server, lokasi FROM Server');
    const now = new Date();

    let message = '<b>🗂️ DASHBOARD AKUN RESELLER</b>\n';
    message += '━━━━━━━━━━━━━━━━━━━━━━\n';

    const grouped = {};
    accounts.forEach(acc => {
      // Cocokkan layanan (Nama Server) dengan data di tabel Server untuk ambil lokasinya
      const s = servers.find(srv => srv.nama_server === acc.layanan);

      const nama_server = acc.layanan; // Ambil langsung dari kolom layanan
      const lokasi = s ? (s.lokasi || 'ID') : 'Global';
      const headerKey = `<b>🖥️ Server: ${escapeHTML(nama_server)} [${escapeHTML(lokasi)}]</b>`;

      if (!grouped[headerKey]) grouped[headerKey] = [];
      grouped[headerKey].push(acc);
    });

    for (const [header, list] of Object.entries(grouped)) {
      message += `\n${header}\n`; 
      message += `──────────────────────\n`;

      list.forEach(acc => {
        // Hitung akumulasi expired: Tanggal beli pertama + total hari (beli + renew)
        const startDate = new Date(acc.tgl_awal);
        const expiredDate = new Date(startDate);
        expiredDate.setDate(expiredDate.getDate() + parseInt(acc.total_hari));
        
        const diffTime = expiredDate - now;
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        
        let statusLabel = diffDays <= 0 ? '🔴 <b>OFF</b>' : (diffDays <= 3 ? '⏳ <b>LIMIT</b>' : '🔵 <b>LIVE</b>');
        let sisaInfo = diffDays > 0 ? ` (<i>${diffDays} Hari</i>)` : "";

        const expDateString = expiredDate.toLocaleDateString('id-ID', { 
          day: '2-digit', month: 'short', year: 'numeric' 
        });

        // DIAKURKAN: Ambil langsung dari kolom protocol hasil INSERT tadi
        // Fallback ke kata depan layanan jika data lama protocol-nya masih kosong (null)
        const protocolType = (acc.protocol || acc.layanan.split(' ')[0]).toUpperCase();

        message += `🎫 <code>${escapeHTML(acc.akun).padEnd(10)}</code> | [<b>${protocolType}</b>]\n`;
        message += `🏷️ Status : ${statusLabel}\n`;
        message += `📆 Expired: <code>${expDateString}</code>${sisaInfo}\n\n`;
      });
    }
    ...
    logger.error('Error reseller_list_akun: ' + err.message);
  }
});
...
bot.action(/^trial_server_zivpn_(\d+)$/, async (ctx) => {
  ...
  try {
    // ==========================
    // 1️⃣ CEK LIMIT TRIAL & SALDO (Min 2rb)
    // ==========================
    const check = await canTakeTrial(String(userId));
    if (!check.allowed) {
      // MODIFIKASI: Cek minimal saldo 2000
      if (check.reason === 'SALDO_INSUFFICIENT') {
        return bot.telegram.sendMessage(
          chatId,
          `⚠️ *AKSES DITOLAK*\n\nMaaf bro, untuk mencoba fitur trial minimal kamu harus memiliki saldo *Rp2.000* di akunmu.\n\n_Saldo tidak akan terpotong, ini hanya syarat keamanan._`,
          { parse_mode: 'Markdown' }
        );
      }
    ...
    const { nama_server, domain, auth } = serverRow;
    ...
    try {
      ...
      console.error('API trial ZIVPN error:', e.message);
    ...
    if (!apiRes.data || apiRes.data.status !== 'success') {
      return bot.telegram.sendMessage(
        chatId,
        `❌ Gagal membuat trial ZIVPN.\n\nDetail: ${apiRes.data?.message || 'Unknown error'}`
      );
    }
    ...
    // ==========================
    // 4️⃣ CLAIM ATOMIK (DENGAN PROTEKSI SALDO)
    // ==========================
    const claim = await claimTrialAtomic(String(userId));
    if (!claim.ok) {
      if (claim.reason === 'SALDO_INSUFFICIENT') {
        return bot.telegram.sendMessage(
          chatId,
          '⚠️ Gagal! Saldo kamu minimal harus Rp2.000 untuk klaim trial.'
        );
      }
      return bot.telegram.sendMessage(
        chatId,
        '⚠️ Trial ditolak (limit tercapai atau race condition).'
      );
    }
    ...
    await dbRunAsync(
      ...
      [userId, d.password, 'zivpn']
    );

    // ==========================
    // 6️⃣ FORMAT PESAN USER
    // ==========================
    const msg = `
🌟 *AKUN ZIVPN TRIAL* 🌟
...
╰───────────────────────
   ✅ <i>Trial Berhasil Dibuat</i>
    ...
    await bot.telegram.sendMessage(
      chatId,
      '❌ Terjadi kesalahan saat proses trial ZIVPN.'
    );
  }
});
...
bot.action(/^trial_server_ssh_(\d+)$/, async (ctx) => {
  ...
  // Perbaikan mention agar lebih rapi
  const mention = ctx.from.username
    ? `@${ctx.from.username}`
    : ctx.from.first_name || 'User';
  ...
  try {
    // ---------- 1) CEK LIMIT TRIAL & SALDO (Min 2rb) ----------
    const check = await canTakeTrial(String(userId));
    if (!check.allowed) {
      // MODIFIKASI: Cek minimal saldo 2000
      if (check.reason === 'SALDO_INSUFFICIENT') {
        return bot.telegram.sendMessage(
          chatId,
          `⚠️ *AKSES DITOLAK*\n\nMaaf bro, fitur trial hanya tersedia jika kamu memiliki saldo minimal *Rp2.000*.\n\n_Saldo tidak akan terpotong, ini hanya syarat keamanan._`,
          { parse_mode: 'Markdown' }
        );
      }
    ...
    // ---------- 4) CLAIM ATOMIK (DENGAN PROTEKSI SALDO) ----------
    const claim = await claimTrialAtomic(String(userId));
    if (!claim.ok) {
      // Tangani jika saldo tidak cukup saat eksekusi atomic
      if (claim.reason === 'SALDO_INSUFFICIENT') {
        return bot.telegram.sendMessage(chatId, '⚠️ Gagal! Saldo minimal harus Rp2.000 untuk klaim trial.');
      }
  ...
  try {
    ...
    if (!check.allowed) {
      ...
      if (check.reason === 'SALDO_INSUFFICIENT') {
        return bot.telegram.sendMessage(
          chatId,
          `⚠️ *AKSES DITOLAK*\n\nMaaf bro, fitur trial hanya tersedia jika kamu memiliki saldo minimal *Rp2.000* di akunmu.\n\n_Saldo tidak akan terpotong, ini hanya syarat keamanan._`,
          { parse_mode: 'Markdown' }
        );
      }
    ...
    // ---------- 4) CLAIM ATOMIK (DOUBLE CHECK SALDO) ----------
    const claim = await claimTrialAtomic(String(userId));
    if (!claim.ok) {
      if (claim.reason === 'SALDO_INSUFFICIENT') {
        return bot.telegram.sendMessage(chatId, '⚠️ Gagal! Saldo kamu minimal harus Rp2.000 untuk klaim trial.');
      }
      return bot.telegram.sendMessage(chatId, '⚠️ Trial ditolak (limit atau race). Coba lagi nanti.');
    }
...
📄 *Save Account:* https://${domainOut}:81/vmess-${username}.txt
...
bot.action(/^trial_server_vless_(\d+)$/, async (ctx) => {
  ...
  // Format mention agar rapi di notifikasi grup
  const mention = ctx.from.username 
    ? `@${ctx.from.username}` 
    : ctx.from.first_name || 'User';
  ...
  try {
    ...
    try {
      ...
      return bot.telegram.sendMessage(chatId, '❌ Gagal menghubungi API trial VLESS di server.');
    }
...
bot.action(/^trial_server_trojan_(\d+)$/, async (ctx) => {
  ...
  // Format mention agar rapi dan klikable di grup
  const mention = ctx.from.username 
    ? `@${ctx.from.username}` 
    : ctx.from.first_name || 'User';
  ...
  try {
    ...
    try {
      ...
      return bot.telegram.sendMessage(chatId, '❌ Gagal menghubungi API trial TROJAN di server.');
    }
...
bot.on('text', async (ctx) => {
  ...
  try {
    ...
    // =====================================
    // 1️⃣ ZIVPN USERNAME BYPASS (PALING ATAS)
    // =====================================
    if (state.type === 'zivpn' && state.step === 'username_create_zivpn') {
      if (!/^[a-zA-Z0-9]{3,20}$/.test(text)) {
        return ctx.reply(
          '❌ *Password ZIVPN harus huruf kecil & angka (3–8 karakter)*',
          { parse_mode: 'Markdown' }
        );
      }

      state.username = text;   // dummy (buat DB & invoice)
      state.password = text;   // 🔥 DIPAKAI API
    ...
    // =====================================
    // 2️⃣ USERNAME GENERIC (NON-ZIVPN)
    // =====================================
    if (
    ...
    // =====================================
    // RENEW ZIVPN - INPUT PASSWORD
    // =====================================
    if (
      state.type === 'zivpn' &&
      state.action === 'renew' &&
      state.step === 'username_renew_zivpn'
    ) {
      if (!/^[a-zA-Z0-9]{3,20}$/.test(text)) {
        return ctx.reply(
          '❌ *Password ZIVPN harus huruf kecil & angka (3–8 karakter)*',
      ...
      state.username = text;   // dummy
      state.password = text;   // REAL
    ...
    // =====================================
    // 4️⃣ EXPIRED DAYS (SEMUA SERVICE)
    // =====================================
    if (state.step.startsWith('exp_')) {
      ...
      // Kita ganti variabel 'type' menjadi 'server.nama_server' agar yang tersimpan adalah nama servernya (Singapore/Indonesia)
await dbRunAsync(`
  INSERT INTO invoice_log (user_id, username, layanan, akun, hari, harga, komisi, protocol, created_at)
  VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
`, [
  userId, 
  ctx.from.username || ctx.from.first_name, 
  server.nama_server, // Tetap Nama Server
  username, 
  days, 
  totalHarga, 
  komisi,
  type.toUpperCase()  // <--- INI KOLOM BARU UNTUK PROTOKOL (SSH/VMESS dll)
]);
      ...
      const actionLabel = action === 'renew' ? 'RENEW' : 'CREATE';
...
async function processMatchingPayment(deposit, matchingTransaction, uniqueCode) {
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      // 1. Cek dulu apakah transaksi sudah pernah diproses
          ...
          db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', 
            ...
            async function(err) {
              ...
              try {
                ...
                usernameLog = userInfo.username ? `@${userInfo.username}` : (userInfo.first_name || String(deposit.userId));
              } catch (e) {}

            // Ganti baris INSERT lama dengan ini:
           db.run(
          'INSERT INTO topup_log (user_id, username, amount, reference, metode, created_at) VALUES (?, ?, ?, ?, ?, datetime("now"))',
          [deposit.userId, usernameLog, deposit.originalAmount, matchingTransaction.reference_id || uniqueCode, 'Orkut (QRIS)']
           );
              ...
              db.run(
                ...
                (err) => {
                  ...
                  // 5. Ambil saldo terbaru & Kirim Notif
                  db.get('SELECT saldo FROM users WHERE user_id = ?', [deposit.userId], async (err, user) => {
                    ...
                    if (notificationSent) {
                      ...
                      // Kirim Notif ke Grup - DENGAN GARIS KEREN
                      try {
                        const timestamp = new Date().toLocaleString('id-ID', { 
                          timeZone: 'Asia/Jakarta',
                          day: '2-digit',
                        ...
                        const referenceId = matchingTransaction.reference_id || uniqueCode;
                        const formattedAmount = deposit.originalAmount.toLocaleString('id-ID');
                        const formattedBalance = user.saldo.toLocaleString('id-ID');
...
│
├─ 💵 <b>Detail Pembayaran</b>
│  ├ Nominal Top Up : Rp ${formattedAmount}
│  └ Saldo Terbaru  : Rp ${formattedBalance}
│
├─ 📋 <b>Referensi</b>
...
╰───────────────────────
   ✅ <i>Pembayaran Dikonfirmasi</i>
                        ...
                        logger.error('Error sending group notification:', e);
                      }

                      db.run('COMMIT'); // SIMPAN SEMUA PERUBAHAN
                      ...
                      db.run('ROLLBACK');
                      reject(new Error('Failed notification'));
                    }
                  });
                }
              );
