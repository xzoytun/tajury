...
async function renewtrojan(username, exp, quota, limitip, serverId) {
  console.log(`⚙️ Renewing TROJAN for ${username} | Exp: ${exp} days | Quota: ${quota} | IP Limit: ${limitip}`);
...
`.trim();
        ...
        .catch((e) => {
          console.error('❌ Trojan Renew Error:', e.message);
          resolve({ status: 'error', message: '❌ Gagal menghubungi server Trojan.' });
        });
    });
  });
}
