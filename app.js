...
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    const serverCards = currentServers.map(s => {
      ...
      return `
<b>🌏 SERVER: ${escapeHTML(s.nama_server)}</b>
<blockquote>Lokasi: <i>${escapeHTML(s.lokasi || 'Global')}</i>
${pricingInfo}
• Limit IP: <code>${s.iplimit || 1}</code> | Quota: <code>${quotaMonth}GB</code>
• Status: <b>${status}</b> (${s.total_create_akun || 0}/${s.batas_create_akun || 0})</blockquote>`.trim();
    }).join('\n');

    let headerText = `━━━━━━━━━━━━━━━━━━━━━━\n<b>📋 LIST SERVER (${currentPage + 1}/${totalPages})</b>\n━━━━━━━━━━━━━━━━━━━━━━\n`;
    if (isReseller) {
