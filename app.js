...
━━━━━━━━━━━━━━━━━━━━━━━━━
📋 <b>Menu Tersedia</b>
<blockquote>• <b>Buat Akun</b> : SSH-VMess-VLess-Trojan-ZiVPN
...
async function showTrialServerMenu(ctx, jenis, page = 0) {
  try {
    ...
    // --- 4. FORMAT SERVER CARDS (KONSISTEN DENGAN PREMIUM) ---
    const serverCards = currentServers.map(s => {
      ...
      return `
${flag} <b>SERVER: ${escapeHtml(s.nama_server)}</b>
🖥️ <b>Host/IP:</b> <code>${escapeHtml(s.domain || '0.0.0.0')}</code>
📍 <b>Lokasi:</b> <i>${escapeHtml(s.lokasi || 'Global')}</i>
...
📊 <b>Status:</b> ${status} (${s.total_create_akun || 0}/${s.batas_create_akun || 0})`.trim();
    ...
    await ctx.editMessageText(headerText + serverCards + footerText, {
      ...
      reply_markup: { inline_keyboard: keyboard }
        ...
        console.error("Error updating trial menu:", e.message);
    });
...
function generateServiceMenu(action) {
  ...
  const commonKeys = [[{ text: '⬅️ Kembali ke Menu Utama', callback_data: 'send_main_menu' }]];
  
  const menuConfig = {
    create: {
      title: '✨ BUAT AKUN PREMIUM',
      desc: 'Pilih protokol yang ingin dibuat. Semua server menggunakan port standar dan mendukung semua ISP.'
    },
    renew: {
      title: '🔄 PERPANJANG AKUN',
      desc: 'Pilih jenis protokol akun Anda. Masa aktif akan otomatis bertambah tanpa perlu setting ulang.'
    },
    trial: {
      title: '⏳ TRIAL GRATIS (1 JAM)',
      desc: 'Coba kualitas server kami secara gratis sebelum membeli. Kecepatan tetap Full Speed!'
    }
  };
...
<b>📑 Pilih Protokol:</b>`.trim();

  // Menggunakan emoji yang lebih "matching" dan profesional
  keyboard = [
    [
    ...
    [
      ...
      { text: '🛡️ Trojan', callback_data: `${action}_trojan` }
    ],
    [
...
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    // --- 4. BUILD KEYBOARD DENGAN BENDERA OTOMATIS ---
    const keyboard = [];
    for (let i = 0; i < currentServers.length; i += 2) {
      const row = [];
      
      // Ambil bendera untuk server pertama di baris ini
      const flag1 = getFlag(currentServers[i].lokasi);
      row.push({ 
        text: `${flag1} ${currentServers[i].nama_server}`, 
      ...
      // Ambil bendera untuk server kedua di baris ini (jika ada)
      if (currentServers[i + 1]) {
        const flag2 = getFlag(currentServers[i + 1].lokasi);
        row.push({ 
          text: `${flag2} ${currentServers[i + 1].nama_server}`, 
    ...
    const serverCards = currentServers.map(s => {
      ...
      let pricingInfo = '';
      if (isReseller) {
        pricingInfo = `💰 <b>Jual:</b> <code>Rp${basePriceDaily.toLocaleString('id-ID')}</code> | <code>Rp${basePriceMonthly.toLocaleString('id-ID')}</code>\n🏷️ <b>Beli:</b> <b>Rp${modalPriceDaily.toLocaleString('id-ID')}</b> | <b>Rp${modalPriceMonthly.toLocaleString('id-ID')}</b>`;
      } else {
        pricingInfo = `💰 <b>Harga:</b> <b>Rp${basePriceDaily.toLocaleString('id-ID')}/hari</b> | <b>Rp${basePriceMonthly.toLocaleString('id-ID')}/bln</b>`;
      }
...
${pricingInfo}
🔐 <b>Limit IP:</b> <code>${s.iplimit || 1}</code>
📦 <b>Quota:</b> <code>${quotaMonth}GB/Bln</code>
📊 <b>Status:</b> ${status} (${s.total_create_akun || 0}/${s.batas_create_akun || 0})`.trim();
    }).join('\n\n');
    ...
    await ctx.editMessageText(headerText + serverCards + footerText, {
      ...
      reply_markup: { inline_keyboard: keyboard }
        ...
        console.error("Gagal edit message:", e.message);
    });
...
`.trim();
    ...
    // 4. Keyboard Navigasi
    const navButtons = [];
    if (page > 1) navButtons.push({ text: '⬅️ Prev', callback_data: `admin_listuser_${page - 1}` });
    if (page < totalPages) navButtons.push({ text: 'Next ➡️', callback_data: `admin_listuser_${page + 1}` });
