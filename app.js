...
async function canTakeTrial(userId) {
  // --- TAMBAHAN: Cek riwayat top up ---
  const topupRecord = await dbGetAsync(
    'SELECT id FROM topup_log WHERE user_id = ? LIMIT 1',
    [userId]
  );

  if (!topupRecord) {
    return { allowed: false, reason: 'BELUM_TOPUP' };
  }
  // --- SELESAI TAMBAHAN ---
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN IMMEDIATE', async (err) => {
        ...
        // --- TAMBAHAN: Validasi Top Up di dalam Transaksi ---
        db.get('SELECT id FROM topup_log WHERE user_id = ? LIMIT 1', [userId], (errTopup, topup) => {
          if (errTopup || !topup) {
            db.run('ROLLBACK');
            return resolve({ ok: false, reason: 'BELUM_TOPUP' });
          }

          // Lanjutkan pengecekan user seperti biasa
          db.get(
            'SELECT role, last_trial_date, trial_count_today FROM users WHERE user_id = ?',
            [userId],
            async (err, user) => {
...
async function sendMainMenu(ctx) {
  ...
  try {
    ...
    saldo = user?.saldo || 0;
    role = user?.role || 'user';
    // Gunakan 'default' atau 'silver' jika level kosong, pastikan tidak crash
  ...
  const now = new Date();
  // Format tanggal: Selasa, 16 Desember 2025
  const dateFormatted = now.toLocaleDateString('id-ID', {
    weekday: 'long', 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric', 
    timeZone: 'Asia/Jakarta' // Pastikan ini zona waktu server Anda
  });
  // Format waktu: 01:27:00 WIB
  const timeFormatted = now.toLocaleTimeString('id-ID', {
    hour: '2-digit', 
    minute: '2-digit', 
    second: '2-digit', 
    timeZone: 'Asia/Jakarta', // Pastikan ini zona waktu server Anda
    timeZoneName: 'short' // Tambahkan WIB/WIT/WITA
  });
  ...
  const text = `
<b>📂 ${namaStore} STORE</b>
<i>Premium VPN Service - Fast, Secure & Reliable</i>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<b>📋 INFORMASI AKUN</b>
• Status  : <code>${roleLabel}</code>
• Saldo   : <code>Rp${saldoFormatted}</code>
• User ID : <code>${userId}</code>
• Transaksi : <code>${totalAkunDibuat}x</code>
• Total User : <code>${totalUserBot}</code>
• Tanggal : <code>${dateFormatted}</code>
• Waktu   : <code>${timeFormatted}</code>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<b>📕 Navigasi & Penawaran Khusus</b>

• Mulai proses pembuatan akun baru (SSH,
  VMess, VLess, Trojan, ZiVPN).
• Buat akun uji coba gratis 1 jam.
• Perpanjang masa aktif akun yang sudah ada.
• Isi saldo untuk kemudahan transaksi.
• Menu khusus untuk melihat komisi dan statistik
  penjualan (Khusus Reseller).
━━━━━━━━━━━━━━━━━━━━━━━━━━━

Bantuan: ${escapeHtml(ADMIN_USERNAME)}
`.trim();
...
otomatis tanpa perlu konfigurasi ulang.
...
<i>60 menit akses penuh • Tanpa biaya • Otomatis</i>

Gunakan akun trial untuk mencoba langsung kualitas layanan kami.
Anda dapat menguji kecepatan koneksi, kestabilan server, dan performa
jaringan sebelum memutuskan menggunakan layanan premium.

Pilih layanan trial yang ingin Anda gunakan:`;
  ...
  try {
    ...
    const keyboard = {
      inline_keyboard: [
        ...
        [{ text: 'Menu Sistem', callback_data: 'admin_system_menu' }],
...
bot.action('admin_system_menu', async (ctx) => {
  ...
  inline_keyboard: [
    ...
    [
      { text: 'Restore DB', callback_data: 'admin_restore2_db' },
      { text: 'All Backup', callback_data: 'admin_restore_all' }
    ],

    // 🔼🔽 Reseller Role
    [
      { text: 'Up Reseller', callback_data: 'admin_promote_reseller' },
      { text: 'Down Reseller', callback_data: 'admin_downgrade_reseller' }
    ],
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<b>📋INFORMASI AKUN</b>
Status       : <code>Reseller Aktif</code>
Level        : <code>${escapeHtml(levelFormatted)}</code>
Tanggal      : <code>${escapeHtml(currentDate)}</code>
Waktu        : <code>${escapeHtml(currentTime)}</code>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<b>📝INFORMASI KOMISI</b>

• Komisi di-reset setiap <b>2 bulan sekali</b>
• Reset berikutnya: <b>${escapeHtml(nextResetStr)}</b>
• Komisi aktif akan menjadi 0 setelah reset
• Kumpulkan komisi untuk upgrade level
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<b>📕FITUR TERSEDIA</b>

• Lihat statistik & riwayat transaksi
• Cek total komisi yang diperoleh
• Export data komisi ke file
• Pantau ranking top reseller
━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
bot.action(/^trial_server_zivpn_(\d+)$/, async (ctx) => {
  ...
  try {
    // ==========================
    // 1️⃣ CEK LIMIT TRIAL & STATUS TOPUP
    // ==========================
    const check = await canTakeTrial(String(userId));
    if (!check.allowed) {
      // MODIFIKASI DISINI: Cek alasan penolakan
      if (check.reason === 'BELUM_TOPUP') {
        return bot.telegram.sendMessage(
          chatId,
          `⚠️ *AKSES DITOLAK*\n\nMaaf bro, fitur trial hanya tersedia untuk pengguna yang sudah pernah melakukan *Top Up* minimal 1x.\n\nSilakan isi saldo terlebih dahulu untuk mencoba layanan kami.`,
          { parse_mode: 'Markdown' }
        );
      }
    ...
    // ==========================
    // 4️⃣ CLAIM ATOMIK (DENGAN PROTEKSI ULANG)
    // ==========================
    const claim = await claimTrialAtomic(String(userId));
    if (!claim.ok) {
      // MODIFIKASI DISINI: Tangani jika di claim atomic juga gagal karena topup
      if (claim.reason === 'BELUM_TOPUP') {
        return bot.telegram.sendMessage(
          chatId,
          '⚠️ Gagal! Kamu harus pernah top up saldo minimal 1x sebelum klaim trial.'
        );
      }
      return bot.telegram.sendMessage(
        chatId,
...
bot.action(/^trial_server_ssh_(\d+)$/, async (ctx) => {
  ...
  try {
    // ---------- 1) CEK LIMIT TRIAL & STATUS TOPUP ----------
    const check = await canTakeTrial(String(userId));
    if (!check.allowed) {
      // TAMBAHAN: Cek jika alasan ditolak karena belum top up
      if (check.reason === 'BELUM_TOPUP') {
        return bot.telegram.sendMessage(
          chatId,
          `⚠️ *AKSES DITOLAK*\n\nMaaf bro, fitur trial hanya tersedia untuk pengguna yang sudah pernah melakukan *Top Up* minimal 1x.\n\nSilakan isi saldo terlebih dahulu untuk mencoba layanan kami.`,
    ...
    // ---------- 4) CLAIM ATOMIK (DENGAN PROTEKSI TOPUP) ----------
    const claim = await claimTrialAtomic(String(userId));
    if (!claim.ok) {
      // Tangani jika di claim atomic juga gagal karena faktor topup (Double Check)
      if (claim.reason === 'BELUM_TOPUP') {
        return bot.telegram.sendMessage(chatId, '⚠️ Gagal! Kamu harus pernah top up saldo minimal 1x.');
      }
...
bot.action(/^trial_server_vmess_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const check = await canTakeTrial(String(userId));
    if (!check.allowed) {
      // TAMBAHAN: Jika ditolak karena belum pernah top up
      if (check.reason === 'BELUM_TOPUP') {
        return bot.telegram.sendMessage(
          chatId,
          `⚠️ *AKSES DITOLAK*\n\nMaaf bro, fitur trial hanya tersedia untuk pengguna yang sudah pernah melakukan *Top Up* minimal 1x.\n\nSilakan isi saldo terlebih dahulu untuk mencoba layanan kami.`,
    ...
    // ---------- 4) CLAIM ATOMIK (DOUBLE CHECK TOPUP) ----------
    const claim = await claimTrialAtomic(String(userId));
    if (!claim.ok) {
      if (claim.reason === 'BELUM_TOPUP') {
        return bot.telegram.sendMessage(chatId, '⚠️ Gagal! Kamu harus pernah top up saldo minimal 1x.');
...
bot.action(/^trial_server_vless_(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const check = await canTakeTrial(String(userId));
    if (!check.allowed) {
      // TAMBAHAN: Cek jika alasan ditolak karena belum pernah top up
      if (check.reason === 'BELUM_TOPUP') {
        return bot.telegram.sendMessage(
          chatId,
          `⚠️ *AKSES DITOLAK*\n\nMaaf bro, fitur trial hanya tersedia untuk pengguna yang sudah pernah melakukan *Top Up* minimal 1x.\n\nSilakan isi saldo terlebih dahulu untuk mencoba layanan kami.`,
  ...
  `Pilih gateway sesuai kenyamanan kamu.`,
  {
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [
          { text: 'Gunakan Pakasir (Otomatis)', callback_data: 'topup_saldo_pakasir' }
        ],
        [
          { text: 'Batalkan', callback_data: 'send_main_menu' }
        ]
