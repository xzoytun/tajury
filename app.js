...
const midtransConfig = {
  isProduction: vars.MIDTRANS_IS_PRODUCTION === true || vars.MIDTRANS_IS_PRODUCTION === 'true',
  serverKey: vars.MIDTRANS_SERVER_KEY,
  clientKey: vars.MIDTRANS_CLIENT_KEY,
  merchantId: vars.MIDTRANS_MERCHANT_ID || '', // Opsional, untuk referensi/logging
};

// Snap instance untuk membuat transaksi
...
const MIN_DEPOSIT_AMOUNT = Number(vars.MIN_DEPOSIT_AMOUNT) || 10;
...
`);
// ========== TABEL MIDTRANS ==========
db.run(`CREATE TABLE IF NOT EXISTS pending_deposits_midtrans (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER,
    ...
    status TEXT DEFAULT 'pending',
    payment_data TEXT,
    source TEXT DEFAULT 'telegram',   -- 'telegram' atau 'web'
    expired_at TEXT,
    created_at TEXT DEFAULT CURRENT_TIMESTAMP
)`);
...
// ... setelah db.serialize selesai ...
safeAlter("ALTER TABLE pending_deposits_midtrans ADD COLUMN source TEXT DEFAULT 'telegram'");
safeAlter("ALTER TABLE pending_deposits_midtrans ADD COLUMN expired_at TEXT");
safeAlter("ALTER TABLE pending_deposits_midtrans ADD COLUMN original_amount INTEGER DEFAULT 0");
// ===========================
// 🔄 Cache Global
// ===========================
...
// ─────────────────────────────────────────────────────────────
// Auto-expire pending QRIS Pakasir yang sudah lewat expired_at
// Jalan tiap 1 menit — mencegah invoice lama terus dianggap aktif
// ─────────────────────────────────────────────────────────────
cron.schedule('*/1 * * * *', async () => {
  try {
    const result = await dbRunAsync(
      `UPDATE pending_deposits_pakasir
       SET status = 'expired'
    ...
    if (result.changes > 0) {
      logger.info(`🧹 [Cron] ${result.changes} invoice Pakasir QRIS otomatis di-expired.`);
    }
  } catch (err) {
    logger.error('❌ [Cron] Gagal expire pending_deposits_pakasir: ' + err.message);
  }
...
keyboard.push([
  ...
  { text: '💳 Top Up Saldo', callback_data: 'topup_saldo_midtrans' }
]);
  ...
  try {
    ...
    for (let i = 0; i < categories.length; i += 2) {
      const row = [
        {
          text: categories[i],
          callback_data: `ppob_cat:${categories[i]}`
        }
      ];
      ...
      if (categories[i + 1]) {
        row.push({
          text: categories[i + 1],
          callback_data: `ppob_cat:${categories[i + 1]}`
        });
      }
...
Silakan pilih kategori produk yang ingin dibeli.
`.trim();
    ...
    ctx.answerCbQuery('❌ Gagal memuat menu.', {
      show_alert: true
    });
  }
});
...
bot.action('topup_saldo_pakasir', async (ctx) => {
  try {
    await ctx.answerCbQuery('Fitur top up saldo saat ini sedang ditutup.');
    return safeMenuSend(ctx,
      '<b>🚫 FITUR TOP UP DITUTUP</b>\n\n' +
      'Mohon maaf, seluruh layanan top up saldo saat ini di tutup.\n\n' +
      'Silakan hubungi admin jika memerlukan bantuan lebih lanjut.',
      {
        parse_mode: 'HTML',
        reply_markup: {
          inline_keyboard: [
    ...
    logger.error('Error topup_saldo action: ' + err.message);
  }
});
...
// ─────────────────────────────────────────
// MIDTRANS: REQUEST AMOUNT (DENGAN FALLBACK)
// ─────────────────────────────────────────
if (state.step === 'request_midtrans_amount') {
    const amount = parseInt(text, 10);
    const minDeposit = MIN_DEPOSIT_AMOUNT || 1000;

    if (isNaN(amount) || amount < minDeposit) {
        return ctx.reply(
            `❌ *Nominal tidak valid.* Minimal Rp ${minDeposit.toLocaleString('id-ID')}.`,
            { parse_mode: 'Markdown' }
        );
    }
    ...
    try {
        const existing = await dbGetAsync(
            `SELECT order_id, amount, original_amount, expired_at, payment_data
             FROM pending_deposits_midtrans
             WHERE user_id = ? 
               AND status = 'pending' 
               AND source = 'telegram'
               AND datetime(expired_at) > datetime('now')`,
        ...
        if (existing) {
            const now = Date.now();
            const exp = new Date(existing.expired_at).getTime();
            const remaining = Math.max(0, Math.floor((exp - now) / 60000));

            await ctx.reply(
                `⏳ *Anda masih memiliki transaksi pending!*\n\n` +
                `📋 Invoice: \`${existing.order_id}\`\n` +
                `💳 Total: Rp ${existing.amount.toLocaleString('id-ID')}\n` +
                `⏱️ Sisa waktu: ${remaining} menit\n\n` +
                `Silakan selesaikan pembayaran atau tunggu hingga kadaluarsa.`,
                {
                    parse_mode: 'Markdown',
                    reply_markup: {
                        inline_keyboard: [
                            [
                                { text: '💳 Bayar Sekarang', url: existing.payment_data },
                                { text: '🔄 Cek Status', callback_data: `midtrans_check_status:${existing.order_id}` }
                            ],
                            [
                                { text: '🔙 Kembali ke Menu', callback_data: 'send_main_menu' }
                            ]
        ...
        logger.error('Gagal cek pending Midtrans: ' + err.message);
        // Jika error, biarkan proses lanjut (fallback)
    }

    // Hapus state setelah nominal valid dan tidak ada pending
    delete userState[chatId];

    try {
        const { orderId, redirectUrl, finalAmount, fee, originalAmount } = 
            await createMidtransTransaction(userId, amount, 'telegram');
        ...
        const msg =
            `💳 *Pembayaran Midtrans* 💳\n\n` +
            `📋 Invoice: \`${orderId}\`\n` +
            `💵 *Nominal Top Up:* Rp ${originalAmount.toLocaleString('id-ID')}\n` +
            `🧾 *Biaya Admin:* Rp ${fee.toLocaleString('id-ID')}\n` +
            `💰 *Total Bayar:* *Rp ${finalAmount.toLocaleString('id-ID')}*\n` +
            `⏳ Kadaluarsa: 1 jam\n\n` +
            `🔗 [Klik di sini untuk bayar](${redirectUrl})\n\n` +
            `Setelah pembayaran sukses, saldo *Rp ${originalAmount.toLocaleString('id-ID')}* akan masuk otomatis.`;
        ...
        await ctx.reply(msg, {
            ...
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '🔄 Cek Status', callback_data: `midtrans_check_status:${orderId}` }],
                    [{ text: '🔙 Kembali ke Menu', callback_data: 'send_main_menu' }]
                ]
            }
        });
    } catch (err) {
        logger.error('❌ Gagal buat transaksi Midtrans: ' + err.message);
        await ctx.reply('❌ Gagal membuat pembayaran. Silakan coba lagi nanti.', { parse_mode: 'Markdown' });
    }
    return;
}
    // ─────────────────────────────────────────
...
//midtrans
async function createMidtransTransaction(userId, originalAmount, source = 'telegram') {
    const fee = generateRandomNumber(1, 300);
    const finalAmount = originalAmount + fee;

    const shortSource = source === 'web' ? 'W' : 'T';
    const shortUserId = String(userId).slice(-6);
    const shortTimestamp = Date.now().toString(36);
    const orderId = `ZD${shortSource}${shortUserId}${shortTimestamp}`;

    // Ambil data user buat ditampilkan di Midtrans
    let customerName = `User ${userId}`;
    let customerEmail = source === 'web' ? `web-${userId}@user.com` : `tg-${userId}@telegram.com`;
    ...
    try {
        if (source === 'telegram') {
            ...
            const user = await dbGetAsync(
                'SELECT username, email FROM web_users WHERE id = ?',
                [userId]
            );
            if (user) {
        ...
        logger.warn('Gagal ambil data user untuk Midtrans:', err.message);
    }
    ...
    const params = {
        ...
        enabled_payments: ['qris', 'gopay', 'dana', 'other_qris', 'shopeepay'],
    ...
    try {
        ...
        await dbRunAsync(
            `INSERT INTO pending_deposits_midtrans 
             (user_id, order_id, amount, original_amount, payment_data, source, expired_at)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [userId, orderId, finalAmount, originalAmount, redirectUrl, source, expires]
        );

        return { orderId, token: transaction.token, redirectUrl, finalAmount, fee, originalAmount };
    } catch (err) {
        logger.error('Midtrans createTransaction error: ' + err.message);
        throw err;
    }
}

bot.action('topup_saldo_midtrans', async (ctx) => {
  try {
    await ctx.answerCbQuery();
    userState[ctx.chat.id] = { step: 'request_midtrans_amount' };
    await ctx.reply(
      `💰 *TOP UP SALDO VIA MIDTRANS*\n\n` +
      `Masukkan nominal (min. Rp 1.000):\n` +
      `Contoh: \`10000\``,
      { parse_mode: 'Markdown' }
    );
  } catch (err) {
    logger.error('Error topup_saldo_midtrans: ' + err.message);
  }
});
// ─── CEK STATUS MIDTRANS ─────────────────────────────────────────────
bot.action(/^midtrans_check_status:(.+)$/, async (ctx) => {
    ...
    try {
        const pending = await dbGetAsync(
            `SELECT status, amount, original_amount FROM pending_deposits_midtrans 
             WHERE order_id = ? AND user_id = ?`,
            [orderId, userId]
        );

        if (!pending) {
            return ctx.reply('❌ Transaksi tidak ditemukan atau sudah kedaluwarsa.', { parse_mode: 'Markdown' });
        }
        ...
        if (status === 'success') {
            return ctx.reply(
                `✅ *Pembayaran Berhasil!*\n\n` +
                `💰 Saldo bertambah: *Rp ${pending.original_amount.toLocaleString('id-ID')}*\n` +
                `📋 Ref: \`${orderId}\``,
                { parse_mode: 'Markdown' }
            );
            ...
            return ctx.reply(
                ...
                `📋 Ref: \`${orderId}\`\n` +
                `💳 Total yang harus dibayar: *Rp ${pending.amount.toLocaleString('id-ID')}*`,
                { parse_mode: 'Markdown' }
            );
        } else {
            return ctx.reply(
        ...
        logger.error('Error checking Midtrans status: ' + err.message);
        ctx.reply('❌ Gagal mengecek status. Coba lagi nanti.', { parse_mode: 'Markdown' });
    }
});
...
// Mengambil 10 transaksi terbaru dari semua sumber (topup, VPN, PPOB)
// untuk user yang sedang login (web)
// ───────────────────────────────────────────────────────────────────────
app.get('/api/transactions/recent', checkAuth, async (req, res) => {
  try {
    const userId = req.user.id; // ID dari web_users

    // 1. Ambil data dari topup_log (topup saldo)
    const topups = await dbAllAsync(
      `SELECT 
         'topup' AS type,
         amount,
         amount AS amount_display,
         'Top Up via ' || metode AS description,
         'success' AS status,
         created_at
       FROM topup_log
       WHERE user_id = ?
    ...
    // 2. Ambil data dari web_orders (pembelian VPN)
    const vpnOrders = await dbAllAsync(
      `SELECT 
         'pembelian_vpn' AS type,
         price AS amount,
         -price AS amount_display,
         'Paket ' || product_type || ' - ' || product_name AS description,
         'success' AS status,
         created_at
       FROM web_orders
       WHERE user_id = ?
    ...
    // 3. Ambil data dari ppob_transactions (PPOB)
    //    Hanya ambil yang source = 'web' agar tidak dobel dengan bot
    const ppobOrders = await dbAllAsync(
      `SELECT 
         'ppob' AS type,
         price AS amount,
         -price AS amount_display,
    ...
    let allTransactions = [...topups, ...vpnOrders, ...ppobOrders];
    ...
    console.error('Error GET /api/transactions/recent:', err.message);
    return res.status(500).json({
      success: false,
...
// ───────────────────────────────────────────────────────────────────────
// 📜 GET /api/vpn-history
// Mengambil riwayat pembelian VPN (web_orders) milik user yang login
// ───────────────────────────────────────────────────────────────────────
app.get('/api/vpn-history', checkAuth, async (req, res) => {
  try {
    ...
    console.error('GET /api/vpn-history error:', err.message);
    return res.status(500).json({
      success: false,
      message: 'Server error'
    });
...
// ============================================================
// PATCH: app.post('/api/ppob/buy', checkAuth, async (req, res) => { ... });
// Ganti SELURUH blok handler lama dengan versi di bawah ini.
// ============================================================

app.post('/api/ppob/buy', checkAuth, async (req, res) => {

  const { sku, target, product_name, price } = req.body;
  const userId = req.user.id;
  ...
  try {
    ...
    // AMBIL USER
    // =========================
    const user = await dbGetAsync(`SELECT * FROM web_users WHERE id = ?`, [userId]);
    ...
    // =========================
    // 🔒 VALIDASI RESPONSE — SAMA SEPERTI DI BOT TELEGRAM
    // Kalau response Digiflazz TIDAK menunjukkan submit sukses
    // (misal karena maintenance/cutoff seller), refund LANGSUNG
    // di sini tanpa menunggu webhook yang mungkin telat/hilang.
    // =========================
    const rc = digiData?.rc;
    const statusLower = digiData?.status?.toLowerCase() ?? '';
    ...
    if (!submitSukses) {
      // Refund saldo karena submit dianggap gagal dari awal
      await dbRunAsync(
        `UPDATE web_users SET balance = balance + ? WHERE id = ?`,
        [price, userId]
      );
      ...
      logger.warn(
        `⚠️ [PPOB WEB] Submit gagal untuk ${refId} (user ${userId}): ` +
        `rc=${rc}, status=${digiData?.status}, message=${digiData?.message}`
      );

      return res.json({
        success: false,
        message: digiData?.message || 'Transaksi gagal diproses provider, saldo sudah dikembalikan.'
      });
    }

    // =========================
    // STATUS TRANSAKSI (submit dianggap sukses/pending)
    // =========================
    let trxStatus = 'PENDING';
    if (digiData?.status) {
      trxStatus = String(digiData.status).toUpperCase();
    }
    ...
    await dbRunAsync(
      `INSERT INTO ppob_transactions
       (web_user_id, sku, target, price, status, ref_id, source, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
      [userId, product_name, target, price, trxStatus, refId, 'web']
    );
    // =========================
    // RESPONSE
    // =========================
    ...
    logger.error('PPOB BUY ERROR: ' + (err?.message || err));

    // =========================
    // REFUND JIKA EXCEPTION (network error, timeout, dll)
    // Hanya refund kalau saldo memang sudah terpotong,
    // untuk mencegah refund ganda / refund yang tidak seharusnya.
    // =========================
    try {
      if (saldoSudahDipotong && price && userId) {
        await dbRunAsync(
          `UPDATE web_users SET balance = balance + ? WHERE id = ?`,
          [price, userId]
        );
        logger.info(`↩️ [PPOB WEB] Refund exception untuk user ${userId} sebesar ${price}`);
      }
    } catch (refundErr) {
      logger.error('REFUND ERROR: ' + (refundErr?.message || refundErr));
    }

    return res.json({
      success: false,
      message: 'Server error / gagal hit provider, saldo dikembalikan.'
    });
  }
...
app.post('/webhook/midtrans', async (req, res) => {
    try {
        ...
        const issuer = payload.issuer || payload.acquirer || payload.payment_type || 'Midtrans';

        logger.info(`Midtrans webhook: ${JSON.stringify(payload)}`);

        // Ambil data pending dari database
        const pending = await dbGetAsync(
            `SELECT * FROM pending_deposits_midtrans WHERE order_id = ? AND status = 'pending'`,
            [orderId]
        );

        if (!pending) {
            logger.warn(`Midtrans webhook: order_id ${orderId} not found or already processed.`);
            return res.status(200).send('OK');
        }

        const userId = pending.user_id;
        const source = pending.source || 'telegram';
        const originalAmount = pending.original_amount; // nominal yang diminta user
        const totalAmount = pending.amount;            // total yang dibayar (termasuk fee)
        const fee = totalAmount - originalAmount;      // biaya admin

        const isSuccess = (transactionStatus === 'settlement' || transactionStatus === 'capture');
        const isExpire = (transactionStatus === 'expire' || transactionStatus === 'cancel' || transactionStatus === 'deny');

        if (isSuccess) {
            // Anti double processing
            if (global.processedTransactions.has(orderId)) {
                logger.warn(`Midtrans webhook: ${orderId} already processed.`);
                return res.status(200).send('OK');
            }
            global.processedTransactions.add(orderId);

            // Mulai transaksi DB
            await dbRunAsync('BEGIN TRANSACTION');

            try {
                // Update saldo user dengan originalAmount (bukan totalAmount)
                if (source === 'web') {
                    await dbRunAsync(
                        `UPDATE web_users SET balance = balance + ? WHERE id = ?`,
                ...
                await dbRunAsync(
                    `UPDATE pending_deposits_midtrans SET status = 'success' WHERE order_id = ?`,
                    [orderId]
                );

                // Catat di topup_log dengan originalAmount
                const username = source === 'web' ? `web-${userId}` : `tg-${userId}`;
                await dbRunAsync(
                    `INSERT INTO topup_log (user_id, username, amount, reference, metode, created_at)
                     VALUES (?, ?, ?, ?, ?, datetime('now'))`,
                    [userId, username, originalAmount, orderId, 'Midtrans (QRIS/CC)']
                );
                ...
                if (source === 'telegram') {
                    const saldoBaru = await getUserBalance(userId);
                    const message = 
                        `✅ *Top Up Berhasil!*\n\n` +
                        `💵 *Nominal Top Up:* Rp ${originalAmount.toLocaleString('id-ID')}\n` +
                        `🧾 *Biaya Admin:* Rp ${fee.toLocaleString('id-ID')}\n` +
                        ...
                        `📋 Ref: \`${orderId}\`\n` +
                        `🏦 Metode: ${issuer} (Midtrans)`;
                    await bot.telegram.sendMessage(userId, message, { parse_mode: 'Markdown' })
                        .catch((err) => logger.error('Gagal kirim notif user Midtrans:', err.message));
                }
                ...
                if (GROUP_ID) {
                    ...
                    let groupMsg = '';
                    if (source === 'web') {
                        const userWeb = await dbGetAsync('SELECT email FROM web_users WHERE id = ?', [userId]);
                        const maskedEmail = escapeHtml(maskEmail(userWeb?.email || ''));
                        groupMsg = `
━━━━━━━━━━━━━━━━━━━━━
<b>💳 DEPOSIT NEW SUCCESS </b>
━━━━━━━━━━━━━━━━━━━━━
» <b>User :</b> <code>${maskedEmail}</code>
» <b>Nominal :</b> Rp ${originalAmount.toLocaleString('id-ID')}
» <b>fee   :</b> Rp ${fee.toLocaleString('id-ID')}
» <b>Total   :</b> Rp ${totalAmount.toLocaleString('id-ID')}
» <b>Metode        :</b> ${escapeHtml(issuer)} (QRIS/e-Wallet)
» <b>Ref ID        :</b> <code>${orderId}</code>
━━━━━━━━━━━━━━━━━━━━━
🕒 <b>Waktu :</b> <code>${timestamp} WIB</code>`;
                    } else {
                        const userTg = await dbGetAsync('SELECT username, first_name FROM users WHERE user_id = ?', [userId]);
                        const userMention = escapeHtml(maskUsername(userTg?.username || userTg?.first_name || String(userId)));
                        groupMsg = `
━━━━━━━━━━━━━━━━━━━━━
<b>💳 DEPOSIT NEW SUCCESS</b>
━━━━━━━━━━━━━━━━━━━━━
» <b>User :</b> ${userMention}
» <b>ID   :</b> <code>${maskUserId(String(userId))}</code>
» <b>Nominal :</b> Rp ${originalAmount.toLocaleString('id-ID')}
» <b>fee   :</b> Rp ${fee.toLocaleString('id-ID')}
» <b>Total   :</b> Rp ${totalAmount.toLocaleString('id-ID')}
» <b>Metode        :</b> ${escapeHtml(issuer)} (QRIS/e-Wallet)
...
🕒 <b>Waktu :</b> <code>${timestamp} WIB</code>`;
                    ...
                    await bot.telegram.sendMessage(GROUP_ID, groupMsg, { parse_mode: 'HTML' })
                        .catch((err) => logger.error('Gagal kirim notif grup Midtrans:', err.message));
                }
                ...
                logger.error('Midtrans transaction commit error: ' + txErr.message);
            ...
            await dbRunAsync(
                `UPDATE pending_deposits_midtrans SET status = 'expired' WHERE order_id = ?`,
                [orderId]
            );
            res.status(200).send('OK');
        } else {
            // Status lainnya (misal: pending, deny) – tidak perlu diupdate
            res.status(200).send('OK');
        }

    } catch (err) {
        logger.error('Midtrans webhook error: ' + err.message);
        res.status(500).send('Error');
    }
});
// ==========================================
// 🧾 MIDTRANS TOPUP (WEB)
// ==========================================
app.post('/api/create-topup-midtrans', checkAuth, async (req, res) => {
    try {
        const userId = req.user.id;
        const { amount } = req.body;
        const parsed = parseInt(amount, 10);
        if (isNaN(parsed) || parsed < 1000) {
            return res.json({ success: false, message: 'Minimal Rp 1.000' });
        }

        // Cek pending yang masih aktif
        const existing = await dbGetAsync(
            `SELECT order_id, amount, original_amount, payment_data FROM pending_deposits_midtrans
             WHERE user_id = ? AND status = 'pending' AND source = 'web'
               AND datetime(expired_at) > datetime('now')`,
            [userId]
        );
        ...
        if (existing) {
            ...
            return res.json({
                ...
                amount: existing.amount,          // final
                original_amount: existing.original_amount,
                fee: existing.amount - existing.original_amount,
                payment_url: existing.payment_data,
            });
        }

        const { orderId, redirectUrl, finalAmount, fee, originalAmount } =
            await createMidtransTransaction(userId, parsed, 'web');
        ...
        logger.error('Midtrans web create error: ' + err.message);
        return res.json({ success: false, message: 'Server error' });
    }
});
app.get('/api/midtrans-status/:orderId', checkAuth, async (req, res) => {
    try {
        const orderId = req.params.orderId;
        const userId = req.user.id;

        const row = await dbGetAsync(
            `SELECT status FROM pending_deposits_midtrans
             WHERE order_id = ? AND user_id = ?`,
            [orderId, userId]
        );

        if (!row) return res.json({ success: false, message: 'Transaksi tidak ditemukan' });
