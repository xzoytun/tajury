...
cron.schedule('0 0 * * *', async () => {
  ...
  logger.info("🧹 Cron: Event kadaluarsa telah dinonaktifkan.");
});
...
// Tambahkan tabel event dan pencapaian reseller
db.run(`CREATE TABLE IF NOT EXISTS reseller_events (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  nama_event TEXT,
  target_penjualan INTEGER, -- Jumlah akun yang harus terjual
  bonus_saldo INTEGER,
  start_date TEXT,
  end_date TEXT,
...
async function canTakeTrial(userId) {
  ...
  // LOGIKA RESET 3 HARI
  if (diffDays >= 1) {
    // Sudah lewat 3 hari, jatah reset ke 0
    return { allowed: true, trialCount: 0, maxTrial, role, last };
  } else {
    // Belum lewat 3 hari, cek apakah pemakaian sudah mencapai batas
    const allowed = currentCount < maxTrial;
    return { allowed, trialCount: currentCount, maxTrial, role, last };
  }
}
...
async function claimTrialAtomic(userId) {
  return new Promise((resolve, reject) => {
    db.serialize(async () => {
      ...
      const today = new Date().toISOString().split('T')[0];
      // Jika ini klaim pertama setelah masa tunggu 3 hari, hitungan mulai dari 1
      // Jika masih dalam masa 3 hari, hitungan ditambah 1
...
async function hasEverTopup(userId) {
  const row = await dbGetAsync(
    'SELECT COUNT(*) AS c FROM topup_log WHERE user_id = ?',
    [userId]
  );
  return (row?.c || 0) > 0;
}
...
async function updateEventProgress(userId, days) {
  try {
    ...
    // Jika syarat event kamu memang minimal 15 hari, biarkan. 
    // Tapi jika ingin semua pembelian masuk, turunkan angkanya (misal: 1).
    const MIN_DAYS = 15; 

    if (inputDays < MIN_DAYS) {
      console.log(`[EVENT] User ${userId} beli ${inputDays} hari. Syarat min ${MIN_DAYS}.`);
      return;
    }
    ...
    // Tambahkan progres secara atomis
    await dbRunAsync(`
      INSERT INTO reseller_event_progress (user_id, event_id, current_sales)
      VALUES (?, ?, 1)
      ON CONFLICT(user_id, event_id) 
      DO UPDATE SET current_sales = current_sales + 1
      WHERE is_claimed = 0 AND current_sales < (SELECT target_penjualan FROM reseller_events WHERE id = ?)
    `, [userId, activeEvent.id, activeEvent.id]);
    
    console.log(`[EVENT] Progres User ${userId} berhasil diupdate.`);
  } catch (err) {
...
async function renderRenewPage(ctx, userId, page) {
  ...
  try {
    if (ctx.updateType === 'callback_query') {
      await ctx.answerCbQuery('🔍 Memvalidasi data akun...').catch(() => {});
    }

    // 1. Ambil riwayat akun dari DB (Gunakan LEFT JOIN supaya tetap muncul meski server dihapus)
    const rawAccounts = await dbAllAsync(
      `SELECT i.akun, i.protocol, s.id AS server_id, s.domain, s.auth, s.nama_server
       FROM invoice_log i
       LEFT JOIN Server s ON i.layanan = s.nama_server
       WHERE i.user_id = ?
       GROUP BY LOWER(i.akun)
       ORDER BY i.id DESC`,
      [userId]
    );

    if (!rawAccounts.length) {
      return safeEdit(ctx, '<b>⚠️ TIDAK ADA DATA</b>\nKamu belum memiliki riwayat akun.', {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🔙 KEMBALI', callback_data: 'send_main_menu' }]] }
      });
    }

    // 2. CEK AKTIVITAS KE SERVER SECARA PARALLEL
    const results = await Promise.allSettled(
      rawAccounts.map(acc => {
        if (!acc.domain) return Promise.reject('No Domain'); // Handle jika server tidak ditemukan di tabel Server
        return axios.get(`http://${acc.domain}:5888/list`, {
          params: { type: acc.protocol.toLowerCase(), auth: acc.auth },
          timeout: 4000 
        }).then(res => ({ acc, data: res.data, online: true }))
      })
    );

    const finalAccounts = [];

    // 3. OLAH HASIL (Tetap masukkan ke daftar meskipun server offline/error)
    for (let i = 0; i < rawAccounts.length; i++) {
      ...
      if (res.status === 'fulfilled') {
        ...
        const target = acc.akun.toLowerCase().trim();
        isExist = data?.data?.some(line => {
          const m = line.match(/User:\s*([^\s|]+)/i) || line.match(/^([^\s|]+)/);
          return m && m[1].toLowerCase().trim() === target;
        });
        
        if (!isExist) statusIcon = '⚠️'; // Akun ada di DB tapi tidak ada di server
      } else {
        statusIcon = '🔌'; // Server Offline/Timeout
      }

      // Masukkan ke daftar (Kita tetap munculkan supaya user bisa lapor kalau ada kendala)
      finalAccounts.push({
        ...acc,
        display: `${statusIcon} ${acc.akun} | ${acc.nama_server || 'Unknown'}`
      });
    }

    // 4. PAGINATION
    const totalItems = finalAccounts.length;
    const totalPages = Math.ceil(totalItems / itemsPerPage);
    const currentPage = page > totalPages ? totalPages : (page < 1 ? 1 : page);
    const startIdx = (currentPage - 1) * itemsPerPage;
    const paginatedItems = finalAccounts.slice(startIdx, startIdx + itemsPerPage);
    ...
    // Navigasi
    const navButtons = [];
    if (currentPage > 1) navButtons.push({ text: '◀️ Prev', callback_data: `ren_page_${currentPage - 1}` });
    if (currentPage < totalPages) navButtons.push({ text: 'Next ▶️', callback_data: `ren_page_${currentPage + 1}` });
    ...
    await safeEdit(ctx, 
      `<b>🔄 RENEW AKUN</b>\n` +
      `Daftar akun Anda (Berdasarkan Database & Server):\n\n` +
      `🔌 = Server Offline / RTO\n` +
      `⚠️ = Tidak terdaftar di server\n` +
      `👤 = Akun Aktif\n\n` +
...
async function sendMainMenu(ctx) {
  ...
  [   
    { text: '🔄 Perpanjang', callback_data: 'renew_select' }, 
    { text: '💰 Top Up Saldo', callback_data: 'topup_saldo_orderkuota' }
  ]
);
...
━━━━━━━━━━━━━━━━━━━━━━━━━
📋 <b>Menu Tersedia</b>
<blockquote>• <b>Buat Akun</b> : SSH, VMess, VLess, Trojan, UDP ZiVPN
...
━━━━━━━━━━━━━━━━━━━━━━━━━`.trim();
  ...
  keyboard = [
    [{ text: '🔵 Ssh', callback_data: `${action}_ssh` }, { text: '🟠 Vmess', callback_data: `${action}_vmess` }],
    [{ text: '🟣 Vless', callback_data: `${action}_vless` }, { text: '🟤 Trojan', callback_data: `${action}_trojan` }],
    [{ text: '🟡 Udp-Zivpn', callback_data: `${action}_zivpn` }],
    ...commonKeys
  ];
...
async function showTrialServerMenu(ctx, jenis, page = 0) {
  try {
    ...
    if (!servers || servers.length === 0) {
      return ctx.reply('<b>⚠️ SERVER TIDAK TERSEDIA</b>\n<blockquote>Mohon maaf, saat ini kapasitas server trial sedang penuh atau dalam pemeliharaan rutin.</blockquote>', { parse_mode: 'HTML' });
    }

    // --- 2. SORTING & PAGINATION (Sama dengan Paid) ---
    ...
    for (let i = 0; i < currentServers.length; i += 2) {
      const row = [];
      row.push({ text: `🌐 ${currentServers[i].nama_server}`, callback_data: `trial_server_${jenis}_${currentServers[i].id}` });
      if (currentServers[i + 1]) {
        row.push({ text: `🌐 ${currentServers[i+1].nama_server}`, callback_data: `trial_server_${jenis}_${currentServers[i+1].id}` });
      }
      keyboard.push(row);
    }
    ...
    // --- 4. FORMAT SERVER CARDS (Gaya Card Premium) ---
    const serverCards = currentServers.map(s => {
      ...
      return `
<blockquote>🌐 SERVER: <b>${escapeHTML(s.nama_server)}</b></blockquote>
• <b>Host/IP:</b> <code>${escapeHTML(s.domain || '0.0.0.0')}</code>
• Lokasi: <i>${escapeHTML(s.lokasi || 'Global')}</i>
• Durasi: <b>60 Menit (Trial)</b>
• Limit IP: <code>${s.iplimit || 1}</code>
• Status: <b>${status}</b> (${s.total_create_akun || 0}/${s.batas_create_akun || 0})`.trim();
    }).join('\n\n');

    const headerText = `━━━━━━━━━━━━━━━━━━━━━━━━━\n<b>⌛ TRIAL ${jenis.toUpperCase()} (${currentPage + 1}/${totalPages})</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    const footerText = `\n\n<b>📝 Syarat & Ketentuan:</b>\n• Limit: 1 Akun/Hari/User\n• Dilarang keras aktivitas ilegal (DDOS/Hacking)`;

    await ctx.editMessageText(headerText + serverCards + footerText, {
      parse_mode: 'HTML',
      reply_markup: { inline_keyboard: keyboard }
    }).catch(() => {});
...
// ------- startSelectServer (FIX FULL: Quota per Bulan + ISP + Lokasi) -------
async function startSelectServer(ctx, action, type, page = 0) {
  try {
    ...
    if (!servers || servers.length === 0) {
      return ctx.reply('<b>⚠️ SERVER TIDAK TERSEDIA</b>\n<blockquote>Mohon maaf, saat ini tidak ada server yang aktif.</blockquote>', { parse_mode: 'HTML' });
    }
    ...
    const navButtons = [];
    if (currentPage > 0) navButtons.push({ text: '⬅️ Prev', callback_data: `Maps_${action}_${type}_${currentPage - 1}` });
    if (currentPage < totalPages - 1) navButtons.push({ text: 'Next ➡️', callback_data: `Maps_${action}_${type}_${currentPage + 1}` });
    if (navButtons.length) keyboard.push(navButtons);
    keyboard.push([{ text: '🔙 Kembali Ke Menu', callback_data: 'send_main_menu' }]);

    // --- 5. FORMAT HEADER & SERVER CARDS ---
    
    // mapping nama protokol agar tampil rapi
    const protocolNames = {
        'vmes': 'VMESS',
        'vless': 'VLESS',
        'trojan': 'TROJAN',
    ...
    const serverCards = currentServers.map(s => {
      ...
      let pricingInfo = '';
      if (isReseller) {
        pricingInfo = `• Jual: <code>Rp${basePriceDaily.toLocaleString('id-ID')}</code> | <code>Rp${basePriceMonthly.toLocaleString('id-ID')}</code>\n• Beli: <b>Rp${modalPriceDaily.toLocaleString('id-ID')}</b> | <b>Rp${modalPriceMonthly.toLocaleString('id-ID')}</b>`;
      } else {
        pricingInfo = `• Harga: <b>Rp${basePriceDaily.toLocaleString('id-ID')}/hari</b> | <b>Rp${basePriceMonthly.toLocaleString('id-ID')}/bln</b>`;
      }
...
${pricingInfo}
• Limit IP: <code>${s.iplimit || 1}</code>
• Quota/Bln: <code>${quotaMonth}GB</code>
• Status: <b>${status}</b> (${s.total_create_akun || 0}/${s.batas_create_akun || 0})`.trim();
    }).join('\n\n');

    // HEADER DINAMIS DISINI
    let headerText = `━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    headerText += `<b>📋 LIST SERVER ${currentProtocol} (${currentPage + 1}/${totalPages})</b>\n`;
    headerText += `━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
    
    if (isReseller) {
      headerText += `<i>⭐ Level: Reseller ${resellerLevel.toUpperCase()} (${diskonRate * 100}% Diskon)</i>\n`;
    }

    const footerText = `\n\n<i>Silakan pilih tombol server di bawah untuk melanjutkan.</i>`;
...
bot.action('admin_system_menu', async (ctx) => {
  ...
  inline_keyboard: [
    ...
    [
      { text: '♻️ Restore Database', callback_data: 'admin_restore2_db' }, 
      { text: '📦 All Backup', callback_data: 'admin_restore_all' }
    ],
    [
      { text: '⬆️ Promote Reseller', callback_data: 'admin_promote_reseller' }, 
      { text: '⬇️ Downgrade User', callback_data: 'admin_downgrade_reseller' }
    ],
    [
...
bot.action('admin_manage_event', async (ctx) => {
  ...
  const buttons = [
    ...
    [{ text: '⬅️ Kembali', callback_data: 'admin_system_menu' }]
  ];
...
bot.action('admin_listreseller', async (ctx) => {
  ...
  try {
    ...
    // 2. Query Data (Urutkan dari saldo terbanyak)
    ...
    if (!rows || rows.length === 0) {
      return ctx.editMessageText('?? <b>Belum ada reseller terdaftar.</b>', { 
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '⬅️ KEMBALI', callback_data: 'admin_system_menu' }]] }
      });
    }

    // 3. Format List Reseller (Mewah & Rapi)
...
Saldo: <b>Rp ${row.saldo.toLocaleString('id-ID')}</b></blockquote>`;
    ...
    const text = `
━━━━━━━━━━━━━━━━━━━━━━━━━━━
🏆 <b>DAFTAR RESELLER AKTIF</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
${list}
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Menampilkan 15 Reseller dengan saldo tertinggi.</i>
`.trim();

    // 4. Keyboard Navigasi
    const keyboard = {
      inline_keyboard: [
        [{ text: '🔄 REFRESH', callback_data: 'admin_listreseller' }],
...
bot.action('admin_listuser', async (ctx) => {
  ...
  // Pastikan adminIds sudah terdefinisi di scope atas
  if (!adminIds.includes(userId)) {
  ...
  try {
    // 1. Instant Feedback
    await ctx.answerCbQuery('Memuat daftar pengguna...').catch(() => {});

    // 2. Query Data (Gunakan dbGetAsync atau promise yang sudah ada)
    const rows = await new Promise((resolve, reject) => {
      // Kita ambil 15 saja biar gak kepanjangan di layar HP
      db.all('SELECT user_id, username, role, saldo FROM users ORDER BY saldo DESC LIMIT 15', (err, rows) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
    ...
    if (!rows || rows.length === 0) {
      return ctx.editMessageText('📭 <b>Database Kosong</b>\nBelum ada pengguna yang terdaftar.', { parse_mode: 'HTML' });
    }

    // 3. Format List dengan blockquote & HTML
    const list = rows.map((row, i) => {
      // Kalau username gak ada, tampilin user_id aja biar admin tau itu siapa
const username = row.username ? `@${row.username}` : `User [${row.user_id}]`;

      const roleLabel = row.role.toUpperCase();
      
      return `<b>${i + 1}. ${username}</b>
<blockquote>ID: <code>${row.user_id}</code>
Role: <code>${roleLabel}</code>
Saldo: <b>Rp ${row.saldo.toLocaleString('id-ID')}</b></blockquote>`;
    }).join('\n');

    const text = `
━━━━━━━━━━━━━━━━━━━━━━━━━━━
👥 <b>DAFTAR PENGGUNA (TOP 15)</b>
━━━━━━━━━━━━━━━━━━━━━━━━━━━
${list}
━━━━━━━━━━━━━━━━━━━━━━━━━━━
<i>Menampilkan 15 pengguna dengan saldo tertinggi.</i>
`.trim();

    // 4. Keyboard Navigasi
    const keyboard = {
      inline_keyboard: [
        [{ text: '🔄 REFRESH', callback_data: 'admin_listuser' }],
        [{ text: '⬅️ KEMBALI', callback_data: 'admin_system_menu' }]
      ]
    };
    ...
    ctx.reply('❌ Terjadi kesalahan saat menarik data pengguna.');
  }
});
...
bot.action(/^res_ren:(.+):(.+):(.+)$/, async (ctx) => {
  const type = ctx.match[1].toLowerCase(); // Pastikan lowercase
  ...
  try {
    if (ctx.updateType === 'callback_query') {
      await ctx.answerCbQuery('🔍 Memvalidasi status akun...').catch(() => {});
    }
    ...
    if (!server) {
      return ctx.reply('❌ Server tidak ditemukan atau sudah dihapus.');
    }

    // --- VALIDASI LIVE KE SERVER SEBELUM LANJUT ---
    try {
      const checkServer = await axios.get(`http://${server.domain}:5888/list`, {
        params: { type, auth: server.auth },
        timeout: 5000 // Tunggu maksimal 5 detik
      });
      ...
      if (checkServer.data && checkServer.data.status === 'success') {
        ...
        // JIKA AKUN TIDAK ADA DI SERVER, STOP DI SINI
        if (!isExist) {
          return ctx.reply(`❌ Akun <code>${username}</code> tidak ditemukan di server <b>${server.nama_server}</b>.\n\nKemungkinan akun sudah dihapus permanen karena expired terlalu lama.`, {
            parse_mode: 'HTML',
      ...
      // Jika server offline/RTO saat dicek
      return ctx.reply(`🔌 <b>Server Offline</b>\n\nGagal memvalidasi akun karena server <b>${server.nama_server}</b> sedang gangguan. Silakan coba lagi nanti.`, {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🔙 KEMBALI', callback_data: 'renew_select' }]] }
      });
    }

    // --- JIKA LOLOS VALIDASI, BARU LANJUT KE STATE INPUT HARI ---
    userState[chatId] = {
      action: 'renew',
      type,
      username,
...
<i>Contoh: 30</i>
    ...
    logger.warn('res_ren handler error: ' + err.message);
...
bot.on('text', async (ctx, next) => {
  ...
  try {
    ...
    if (stateKms && adminIds.includes(userId)) {
      ...
      if (stateKms.step === 'WAIT_AMOUNT') {
        ...
        try {
          ...
          // ❌ BARIS DI BAWAH INI DIHAPUS agar saldo utama tidak langsung isi
          // await dbRunAsync("UPDATE users SET saldo = saldo + ? WHERE user_id = ?", [amount, targetId]);
          
          // ✅ Cukup catat di reseller_sales agar "Total Komisi" user bertambah
    ...
    // =================================================================
    if (!stateUser || typeof stateUser !== 'object') return;
  ...
  // TARIK KOMISI MANUAL
  // =================================================================
  if (state && state.step === 'WAIT_CLAIM_NOMINAL') {
    ...
    // Hitung total komisi yang tersedia di tabel reseller_sales
    const komisiRow = await dbGetAsync('SELECT SUM(komisi) AS total FROM reseller_sales WHERE reseller_id = ?', [userId]);
    const totalKomisi = komisiRow?.total || 0;
    ...
    try {
      ...
      // 2. Catat penarikan di tabel reseller_sales dengan nilai MINUS agar saldo komisi berkurang
      await dbRunAsync(
      ...
      // --- 🔥 NOTIFIKASI KE GRUP (STYLE KONSISTEN) ---
      if (typeof GROUP_ID !== 'undefined' && GROUP_ID) {
        const mention = ctx.from.username ? `@${ctx.from.username}` : ctx.from.first_name || 'User';
...
`.trim();
      ...
      delete userState[chatId]; // Hapus state setelah selesai
    ...
    if (state.step && state.step.startsWith('exp_')) {
      ...
      // ✅ Cek bentrok username untuk CREATE
      if (action === 'create') {
      ...
      // 🔥 Hanya tambah kuota server jika CREATE
      if (action === 'create') {
        await dbRunAsync('UPDATE Server SET total_create_akun = total_create_akun + 1 WHERE id = ?', [serverId]);
...
async function checkQRISStatus() {
  try {
    ...
    for (const [uniqueCode, deposit] of pendingDeposits) {
      ...
      // 1. Logika Expired 5 Menit (Sesuai kode kamu)
      ...
      // 2. Cek Mutasi ke Vercel
      try {
        const payload = buildPayload();
        const resultcek = await axios.post(API_URL, payload, { headers, timeout: 10000 });
        
        // Ambil data dari format JSON Vercel
        const history = resultcek.data.qris_history?.results || [];
        
        // Buat list transaksi yang sudah diparse (agar cocok dengan logika find kamu)
        const transaksiList = history.map(t => ({
          tanggal: t.tanggal,
          kredit: Number(t.kredit.toString().replace(/\D/g, '')),
          brand: t.brand?.name || '-',
          id: t.id // Kita pakai ID asli dari Orkut untuk referensi
        }));

        // Cocokkan nominal (expectedAmount)
        const expectedAmount = Number(deposit.amount);
        const matched = transaksiList.find(t => t.kredit === expectedAmount);

        if (matched) {
          // Gunakan ID transaksi dari Orkut sebagai referensi unik
          ...
          if (success) {
            ...
            try {
              ...
              if (isFriday && amt >= BONUS_THRESHOLD) {
                ...
                db.get("SELECT id FROM weekly_bonus_claims WHERE user_id = ? AND claimed_date = ?", [String(deposit.userId), today], (chkErr, row) => {
                  if (!row && !chkErr) {
                    db.run("UPDATE users SET saldo = saldo + ? WHERE user_id = ?", [BONUS_AMOUNT, deposit.userId], (uErr) => {
                      if (!uErr) {
                        const bonusRef = `friday-bonus-${Date.now()}`;
                        db.run("INSERT INTO transactions (user_id, amount, type, reference_id, timestamp) VALUES (?, ?, ?, ?, ?)", [deposit.userId, BONUS_AMOUNT, 'bonus', bonusRef, Date.now()]);
                        db.run("INSERT INTO weekly_bonus_claims (user_id, amount, claimed_date, reference) VALUES (?, ?, ?, ?)", [String(deposit.userId), BONUS_AMOUNT, today, bonusRef]);
                        bot.telegram.sendMessage(deposit.userId, `🎉 Bonus Jumat! Topup Rp${amt.toLocaleString('id-ID')}. Kamu menerima bonus Rp${BONUS_AMOUNT.toLocaleString('id-ID')} 🎁`).catch(e => {});
                      }
                    });
                  }
                });
              }
            } catch (e) { logger.error('Error Friday bonus:', e.message); }

            // Hapus pending deposit setelah sukses
            delete global.pendingDeposits[uniqueCode];
            db.run('DELETE FROM pending_deposits WHERE unique_code = ?', [uniqueCode]);
          }
        }
...
async function processMatchingPayment(deposit, matchingTransaction, uniqueCode) {
  // Pakai ID transaksi dari Orkut (matchingTransaction.id) agar lebih akurat
  const refId = matchingTransaction.id ? `orkut_${matchingTransaction.id}` : uniqueCode;
  const transactionKey = `${refId}_${matchingTransaction.kredit}`;
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      // 1. Cek duplikasi
      db.get('SELECT id FROM transactions WHERE reference_id = ?', [refId], async (err, row) => {
        if (err || row) {
          db.run('ROLLBACK');
          resolve(false);
          return;
        ...
        // 2. Update saldo
        db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [deposit.originalAmount, deposit.userId], async function(err) {
          if (err) { db.run('ROLLBACK'); reject(err); return; }

          // 3. Ambil username
          let usernameLog = String(deposit.userId);
          const userRow = await new Promise((res) => {
            db.get("SELECT username, first_name FROM Users WHERE user_id = ?", [deposit.userId], (err, row) => res(row));
          });
          if (userRow) usernameLog = userRow.username ? `@${userRow.username}` : (userRow.first_name || String(deposit.userId));

          // 4. Log Topup & Transaction
          db.run('INSERT INTO topup_log (user_id, username, amount, reference, metode, created_at) VALUES (?, ?, ?, ?, ?, datetime("now"))', [deposit.userId, usernameLog, deposit.originalAmount, refId, 'Orkut (QRIS)']);
          db.run('INSERT INTO transactions (user_id, amount, type, reference_id, timestamp) VALUES (?, ?, ?, ?, ?)', [deposit.userId, deposit.originalAmount, 'deposit', refId, Date.now()], (err) => {
            if (err) { db.run('ROLLBACK'); reject(err); return; }

            // 5. Notifikasi
            db.get('SELECT saldo FROM users WHERE user_id = ?', [deposit.userId], async (err, user) => {
              if (err || !user) { db.run('ROLLBACK'); resolve(false); return; }

              await sendPaymentSuccessNotification(deposit.userId, deposit, user.saldo).catch(() => {});
              if (deposit.qrMessageId) { try { await bot.telegram.deleteMessage(deposit.userId, deposit.qrMessageId); } catch (e) {} }
...
│  └ Saldo Terbaru  : Rp ${user.saldo.toLocaleString('id-ID')}
│
├─ 📋 <b>Referensi</b>
│  └ <code>${refId}</code>
│
...
╰───────────────────────`.trim();
              ...
              await bot.telegram.sendMessage(GROUP_ID, groupMessage, { parse_mode: 'HTML' }).catch(() => {});
              db.run('COMMIT');
              resolve(true);
            });
          });
...
async function cekMutasiOrkut() {
  try {
    const response = await axios.post(API_URL, buildPayload(), { headers });

    if (response.data?.qris_history?.success) {
      const history = response.data.qris_history.results;

      for (const item of history) {
        if (item.status === 'IN') {
          const nominalKredit = parseInt(item.kredit.replace(/\./g, '')); 
          
          // Membungkus db.get ke dalam Promise supaya bisa di-await
          await new Promise((resolve) => {
            db.get("SELECT * FROM deposit_logs WHERE amount = ? AND status = 'pending'", [nominalKredit], async (err, deposit) => {
              if (err || !deposit) return resolve(); // Lanjut ke mutasi berikutnya

              const depositData = {
                id: deposit.id,
                userId: deposit.user_id,
                amount: deposit.amount,
                originalAmount: deposit.original_amount || deposit.amount,
                qrMessageId: deposit.qr_message_id
              };

              logger.info(`🔍 Mutasi cocok! ID: ${item.id} untuk User: ${depositData.userId}`);
              
              // Tunggu sampai proses payment & kirim pesan selesai
              const success = await processMatchingPayment(depositData, item, `ORKUT_${item.id}`);
              
              if (success) {
                db.run("UPDATE deposit_logs SET status = 'success' WHERE id = ?", [deposit.id], () => {
                  logger.info(`✅ Saldo Berhasil: user_id ${depositData.userId}`);
                  resolve();
                });
              } else {
                resolve();
    ...
    // logger.error('❌ Error Sinkronisasi Orkut: ' + error.message);
  }
}
